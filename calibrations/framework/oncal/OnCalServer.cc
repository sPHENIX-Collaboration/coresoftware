...
void
OnCalServer::registerHisto(TH1 *h1d, OnCal *Calibrator, const int replace)
{
  if (Calibrator)
    {
...
void
OnCalServer::unregisterHisto(const string &calibratorname)
{
...
int
OnCalServer::BeginRun(const int runno)
{
  if (runno <= 0)
    {
  ...
  for (iter = Subsystems.begin(); iter != Subsystems.end(); ++iter)
    {
      ...
      cout << "SERVER SUMMARY: " << oncal->Name() << "  "
           ...
           << (databasecommitstatus == 1 ?
               "DB commit: SUCCESS  " : "DB commit: FAILURE  ")
           << endl;
...
void
OnCalServer::Print(const string &what) const
{
...
void
OnCalServer::RunNumber(const int runnum)
{
  runNum = runnum;
  SetBorTime(runnum);
  if (recordDB)
...
bool
OnCalServer::insertRunNumInDB(const string &DBtable, const int runno)
{
...
bool
OnCalServer::findRunNumInDB(const string &DBtable, const int runno)
{
  if (!DBconnection)
    {
      connectDB();
...
int
OnCalServer::updateDB(const string &table, const string &column,
                      const time_t ticks)
{
  if (!DBconnection)
    {
...
int
OnCalServer::check_create_subsystable(const std::string &tablename)
{
  if (!connectDB())
    {
...
int
OnCalServer::add_calibrator_to_statustable(const std::string &calibratorname)
{
  if (!connectDB())
    {
      cout << "could not connect to " << database << endl;
...
int
OnCalServer::check_calibrator_in_statustable(const std::string &calibratorname)
{
...
int
OnCalServer::check_create_successtable(const std::string &tablename)
{
  if (!connectDB())
    {
      cout << "could not connect to " << database << endl;
...
void
OnCalServer::recordDataBase(const bool bookkeep)
{
  recordDB = bookkeep;
  if (recordDB)
    {
...
void
OnCalServer::BeginTimeStamp(const PHTimeStamp &TimeStp)
{
...
void
OnCalServer::EndTimeStamp(const PHTimeStamp &TimeStp)
{
...
int
OnCalServer::SyncCalibTimeStampsToOnCal(const OnCal *calibrator, const int commit)
{
...
OnCalServer::SyncCalibTimeStampsToOnCal(const OnCal *calibrator, const std::string &table, const int commit)
{
  ...
  try
    ...
    {
      cout << "Cannot connect to " << "oncal" << endl;
      cout << e.getMessage() << endl;
      return -1;
    }
...
int
OnCalServer::CopyTables(const OnCal *calibrator, const int FromRun, const int ToRun, const int commit) const
{
...
void
OnCalServer::CreateCalibrationUpdateStatus(OnCal *calibrator, const string &table, const string &tablecomment, const int dbcode)
{
...
int
OnCalServer::CopySnglTable(const string &pdbclass, const string &tablename, const int bankid, const int FromRun, const int ToRun, const int commit)
{
...
int
OnCalServer::CopySnglTableNewBankId(const string &pdbclass, const string &tablename, const int bankid, const int Tobankid, const int FromRun, const int ToRun, const int commit)
{
  RunToTime *rt = RunToTime::instance();
...
int
OnCalServer::ClosestGoodRun(OnCal *calibrator, const int irun, const int previous)
{
  RunToTime *rt = RunToTime::instance();
...
int
OnCalServer::OverwriteCalibration(OnCal *calibrator, const int runno, const int commit, const int FromRun)
{
  if (FromRun < 0)
    {
      return -1;
...
int
OnCalServer::FixMissingCalibration(OnCal *calibrator, const int runno, const int commit, const int fromrun)
{
  int iret = -1;
...
int
OnCalServer::SetBorTime(const int runno)
{
//  recoConsts *rc = recoConsts::instance();
  RunToTime *runTime = RunToTime::instance();
...
int
OnCalServer::SetEorTime(const int runno)
{
//  recoConsts *rc = recoConsts::instance();
  RunToTime *runTime = RunToTime::instance();
...
int
OnCalServer::GetRunTimeTicks(const int runno, time_t &borticks, time_t &eorticks)
{
  RunToTime *runTime = RunToTime::instance();
...
int
OnCalServer::requiredCalibration(SubsysReco *reco, const std::string &calibratorname)
{
...
int
OnCalServer::FindClosestCalibratedRun(const int irun)
{
  RunToTime *rt = RunToTime::instance();
  PHTimeStamp *ts = rt->getBeginTime(irun);
  if (!ts)
...
int
OnCalServer::FillRunListFromFileList()
{
...
int
OnCalServer::AdjustRichTimeStampForMultipleRuns()
{
...
int
OnCalServer::GetCalibStatus(const string &calibname, const int runno)
{
  int iret = -3;
  if (!connectDB())
    {
...
void
OnCalServer::TestMode(const int i)
{

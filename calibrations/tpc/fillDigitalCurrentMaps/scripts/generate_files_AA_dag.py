...
work_dir = "/sphenix/user/shulga/Work/TpcPadPlane_phi_coresoftware/coresoftware/calibrations/tpc/fillDigitalCurrentMaps/"
...
ff_all_dag= open("./run_all_AA_jobs_dag.sh","w+")
...
for j, (start,end) in enumerate(zip(evt_start,evt_end)):
    filename_bx = "./condor_macros/run_all_AA_jobs_{}.sh".format(int(evt_bX[j]))
    filename_dag = "./condor_macros/run_all_AA_jobs_{}.dag".format(int(evt_bX[j]))
    ...
    for i in range(start,end+10):
        filename = "./condor_macros/run_files_AA_{}_{}.sh".format(j,i)
        f= open(filename,"w+")
        f.write("#!/usr/bin/bash"+"\n")
        f.write("source macros/run_files_AA.sh {} {} {}".format(i,i+1,evt_bX[j])+"\n")
        f.close
        chmod_px(filename)
        filename_job = "./condor_macros/condor_run_files_AA_{}_{}.job".format(j,i)

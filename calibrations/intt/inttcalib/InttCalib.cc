...
int InttCalib::Init(PHCompositeNode*)
{
  return Fun4AllReturnCodes::EVENT_OK;
}

int InttCalib::InitRun(PHCompositeNode*)
{
  m_evts = 0;
...
int InttCalib::ResetEvent(PHCompositeNode*)
{
  return Fun4AllReturnCodes::EVENT_OK;
}
...
int InttCalib::MakeHotMapPng()
{
  ...
  snprintf(buff, sizeof(buff), "Run: %08d Events: %d", m_run_num, m_evts);
  caption_text.DrawText(0.5, 0.75, buff);

  caption_text.SetTextSize(0.10);
  snprintf(buff, sizeof(buff), "%.3E <= [hitrate] excludes %06.3lf%%", m_min_hitrate, m_min_fraction * 100);
  caption_text.DrawText(0.5, 0.5, buff);
  snprintf(buff, sizeof(buff), "[hitrate] <= %.3E excludes %06.3lf%%", m_max_hitrate, m_max_fraction * 100);
  caption_text.DrawText(0.5, 0.35, buff);
  snprintf(buff, sizeof(buff), "Keeping %.3lf%%", 100.0 - (m_min_fraction + m_max_fraction) * 100.0);
  caption_text.DrawText(0.5, 0.2, buff);
...
int InttCalib::MakeBcoMapPng()
{
  ...
  {
    char buff[256];
	snprintf(buff, sizeof(buff), "bco_pad_%02d", i);
    bco_cnvs->cd();
	...
	for(InttMap::RawData_s raw = raw_begin; raw <= raw_end; ++raw)
	{
      ...
      bco_pdf_pad->cd();
	  snprintf(buff, sizeof(buff), "bco_hist_%03d", h);
      bco_hist[h] = new TH1D ( //
	    buff, buff, //
	    128, -0.5, 127.5 //
      );

      snprintf(buff, sizeof(buff), ";BCO Difference;intt%01d (%01d - %02d)", raw_begin.pid - 3001, raw_begin.fee, raw_end.fee);
      bco_hist[h]->SetTitle(buff);
  ...
  for(int fee = 0; fee < 7; ++fee)
  {
	...
	snprintf(buff, sizeof(buff), "FCh %01d, %02d", fee, fee + 7);
	legend_text.DrawText(0.6, (2.0 * fee + 1.0) / 14.0, buff);
...
int InttCalib::End(PHCompositeNode*)
{
  return Fun4AllReturnCodes::EVENT_OK;
}

int InttCalib::Reset(PHCompositeNode*)
{
  return Fun4AllReturnCodes::EVENT_OK;
}

    ...
    for (int j = 0; j < 256; j++)
     {
      ...
      if (isnan(h_M_tbt[ieta][j]->GetEntries())){
      ...
      h_p0_tbt->SetBinError(ieta + 1, j + 1, fitFunOut[ieta][j]->GetParError(0));
		...
		// updating cdbtree in given eta-bin
    for (int j = 0; j < 256; j++)
     {
      if (use_h_target_mass){
        final_mass_target = h_target_mass->GetBinContent(ieta+1, j+1);
      }
      float correction = final_mass_target / h_peak_tbt->GetBinContent(ieta+1, j+1);
      if (h_peak_tbt->GetBinContent(ieta+1, j+1) == 0){
				correction = 0;
      }
      unsigned int key = TowerInfoDefs::encode_emcal(ieta, j);
      float val1 = cdbttree1->GetFloatValue(key, m_fieldname);
      cdbttree2->SetFloatValue(key, m_fieldname, val1 * correction);
     }

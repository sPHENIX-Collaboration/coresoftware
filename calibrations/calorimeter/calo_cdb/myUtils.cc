...
std::pair<Int_t, Int_t> myUtils::getSectorIB(Int_t iphi, Int_t ieta) {
    Int_t k = iphi / CaloGeometry::CEMC_NTOW_IB_SIDE;

    Int_t sector = (ieta < 48) ? k + 32 : k;
    Int_t ib = (ieta < 48) ? CaloGeometry::CEMC_NIB_PER_SECTOR - (ieta / CaloGeometry::CEMC_NTOW_IB_SIDE) - 1: (ieta - 48) / CaloGeometry::CEMC_NTOW_IB_SIDE;

    return std::make_pair(sector, ib);
}

std::pair<Int_t, Int_t> myUtils::getSectorIB(Int_t towerIndex) {
    Int_t k = towerIndex / CaloGeometry::CEMC_NCHANNEL_PER_SECTOR;

    Int_t sector = (k % 2) ? (k - 1) / 2 : (k / 2) + 32;
    Int_t ib = (k % CaloGeometry::CEMC_NCHANNEL_PER_SECTOR) / CaloGeometry::CEMC_NCHANNEL_PER_IB;

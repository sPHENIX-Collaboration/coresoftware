...
const PHGenEvent* PHGenEventListv1::at(size_t i) const
{
  ...
  const PHGenEvent* genevent = static_cast<const PHGenEvent*>(versioned_const);
  return genevent;
}
...
void PHGenEventListv1::insert(const PHGenEvent* genevent)
{
  ...
  const PHGenEventVersion* versioned_const = dynamic_cast<const PHGenEventVersion*>(genevent);
  if (versioned_const)
  {

...
void
PgPostBankBackupManager::deleteSQLStatement(TSQLStatement * stmt)
{
  if (!stmt)
    {
...
void
PgPostBankBackupManager::deleteODBCPreparedStatement(
    ...
    const std::string &table_name)
{
  ...
  pdbcalchan_classname = string("Pdb")
  ...
  if (rs->Next())
    {
      ...
      PgPostBankBackupStorage * bs = SQLResultSet2BackupStorage(rs, table_name);
...
bool
PgPostBankBackupManager::commit(PgPostBankBackupStorage * bs)
{
  if (!bs)
    {
  ...
  try
    {
      ...
      if (isRIdExist(bs->get_database_header().getTableName(),
        ...
        {
          cout << "PgPostBankBackupManager::commit - Error - " << " RID "
              << bs->get_database_header().getRId()
...
int
PgPostBankBackupManager::commitAllBankfromTFile(const std::string &input_file)
{
              ...
              if (res != 1)
                {
                  ...
                  timer_log.get()->restart();
                  bklog->Log(rid,
                      (PgPostBankBackupLog::enu_ops) (PgPostBankBackupLog::kOptFile2Db
                          * PgPostBankBackupLog::kOptFailed));
                  timer_log.get()->stop();
                }
              else
                {
...
//! Database -> TFile
int
PgPostBankBackupManager::fetchBank2TFile(const std::string &bankName,
...
int
PgPostBankBackupManager::fetchAllBank2TFile(const std::string &bankName,
  ...
  if (row_cnt <= 0)
    {
      cout << "PgPostBankBackupManager::fetchAllBank2TFile - WARNING - "
          << " no data in table " << table_name << endl;
//      return row_cnt;
    }
...
//        << endl;
  ...
  PgPostBankBackupLog bklog(table_name, tag);
  bklog.Init();
  ...
  while (rs->Next())
    {
      ...
      timer_log.get()->restart();
      bklog.Log(rid,
          (PgPostBankBackupLog::enu_ops) (success
              * PgPostBankBackupLog::kOptBackup2File));
      timer_log.get()->stop();
...
//  std::ostringstream t2;
  ...
  tem << "select bankid, description,rid from " << table_name << " where rid = "
      << rid;
...
//  std::ostringstream t2;
  ...
  tem << "select COUNT(*) AS NumberOfRows from " << table_name;
...
//  std::ostringstream t2;
  ...
  tem << "select rid from " << table_name;
...
void
PgPostBankBackupManager::dumpTable(const std::string &bankName,
    std::ostream & out)
{
...
//              << HashPdbCalChan(c) << endl;
          ...
          if (verbosity >= 4)
            {
              cout << "PgPostBankBackupManager::dumpTable -  " << table_name
                  << ":" << rid << ", " << c.GetUniqueID() << ", "
...
int
PgPostBankBackupManager::CleanTable(const std::string &bankName,
    ...
    const std::string & log_file_name)
{
  ...
  vector<int> bankids;
    { // get bank IDs
      ...
      tem << "select bankid from " << table_name
      ...
      if (verbosity >= 1)
        cout << "PgPostBankBackupManager::CleanTable -" << table_name
  ...
  for (vector<int>::const_iterator it = bankids.begin(); it != bankids.end();
      ++it)
    {
      ...
      tem << "select inserttime,startvaltime,endvaltime,rid FROM " << table_name
...
//                  assert(last_starttime<last_endtime);
                  ...
                  const bool contain_startvaltime = startvaltime
                      >= covered_startvaltime
                      && startvaltime <= covered_endvaltime;
                  const bool contain_endvaltime = endvaltime
                      >= covered_startvaltime
...
//                    $covered_period{covered_startvaltime} = endvaltime;
                  ...
                  else if (startvaltime < covered_startvaltime
                      && endvaltime > covered_endvaltime)
                    {
  ...
  // do the deleting
    {
      ...
      ostringstream sqlcmd;
      sqlcmd << "delete from " << table_name << " where rid = ?";
          ...
          if (do_delete)
            {
              ...
              if (res == 0)
                {
                  cout << "PgPostBankBackupManager::CleanTable - Error - "
                      << "DATABASE: delete " << table_name << " failed. "
                      << "Make sure you commit to the master database " << endl;
                  exit(1);
                }
              assert(res == 1);
              ...
              if (do_log)
                {
                  flog << "Deleted Entry - " << table_name << " RId " << rid
                      << endl;
                }

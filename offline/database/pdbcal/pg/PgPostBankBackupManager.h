...
public:
  ...
  //! total row counts
  int
  getTotalRowCount(const std::string & bankName);
  ...
  int
  commitAllBankfromTFile(const std::string &in_file);

  //! Database -> TFile for official operation use
  int
  fetchBank2TFile(const std::string & bankName, const rid_list_t & rid_list,
      const std::string &out_file);

  //! Database -> TFile for all all records satisifying  record_selection
  int
  fetchAllBank2TFile(const std::string & bankName,
  ...
  int
  CleanTable(const std::string & bankName,

...
int QAG4SimulationDistortions::InitRun(PHCompositeNode *topNode)
{
  ...
  if(not m_trackMap or not m_clusterContainer)
    {
...
int QAG4SimulationDistortions::process_event(PHCompositeNode*)
{
  ...
  for(const auto& [key, track] : *m_trackMap)
      ...
      for(auto iter = track->begin_states(); iter!= track->end_states(); ++iter)
	{
	  ...
	  TrkrDefs::cluskey key = std::stoi(state->get_name());
	  ...
	  const Acts::Vector3 stateGlobMom = Acts::Vector3(state->get_px(),
							   state->get_py(),
							   state->get_pz());
	  const Acts::BoundSymMatrix statecov = 
	    transformer.rotateSvtxTrackCovToActs(state);
	  
	  const float stateRPhiErr = std::sqrt(statecov(Acts::eBoundLoc0, Acts::eBoundLoc0))/Acts::UnitConstants::cm;
	  const float stateZErr = sqrt(statecov(Acts::eBoundLoc1, Acts::eBoundLoc1))/Acts::UnitConstants::cm;
...
//____________________________________________________________________________..
int QAG4SimulationDistortions::End(PHCompositeNode*)
{
  return Fun4AllReturnCodes::EVENT_OK;
}

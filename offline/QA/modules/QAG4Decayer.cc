...
//		std::cout << "ParentPDGID  = " << ParentPDGID << "  NParticles = " << NParticles << std::endl;
	...
	*/
		...
		if((ParentTrkId > 0 || abs(gflavor) == abs(ParentPDGID)) && VtxToQA == true){   //abs(gflavor) == abs(ParentPDGID) HF oscillation for instance Bs -> anti-Bs 
			if(NewVtx){
...
//	QATree->Write();
	...
	QAE->Write();
	QACosTheta->Write();
	ProperLifeTime->Write();
	SVtoPVDistance->Write();
	BR1DHis->Write();
	fout->Close();

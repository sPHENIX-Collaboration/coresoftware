...
int InttRawHitQA::InitRun(PHCompositeNode *topNode)
{
  ...
  PHCompositeNode *intt_node = dynamic_cast<PHCompositeNode *>(trkr_itr.findFirst("PHCompositeNode", "INTT"));
  if (!intt_node)
  {
  ...
  while ((thisnode = iter()))
  {
    ...
    if (theNode)
    {
      ...
      if (theNode->getName() == "INTTEVENTHEADER" || thisnode->getName().find("G4") != std::string::npos)
      {
        continue;
      }
  ...
  for (int felix = 0; felix < InttQa::kFelix_num; felix++)
  {
    ...
    hist_fee_chip_chan_[felix] = dynamic_cast<TH3 *>(hm->getHisto(name));

    std::string name_bco_event = name + "_ladder_bco_full_event_counter";
    hist_fee_bco_full_event_counter_[felix] = dynamic_cast<TH3 *>(hm->getHisto(name_bco_event));

    std::string name_bco_event_diff = name + "_ladder_bco_full_event_counter_diff";
    hist_fee_bco_full_event_counter_diff_[felix] = dynamic_cast<TH3 *>(hm->getHisto(name_bco_event_diff));

    std::string name_event_counter = name + "_event_counter";
    hist_event_counter_[felix] = dynamic_cast<TH1 *>(hm->getHisto(name_event_counter));

    std::string name_event_counter_diff = name + "_event_counter_diff";
    hist_event_counter_diff_[felix] = dynamic_cast<TH1 *>(hm->getHisto(name_event_counter_diff));
  }
  ...
  for (int felix = 0; felix < InttQa::kFelix_num; felix++)
  {
    for (int ladder = 0; ladder < InttQa::kFee_num; ladder++)
    {
      ...
      hist_hitmap_[felix][ladder] = dynamic_cast<TH2 *>(hm->getHisto(name));
    }
  }

  hist_nhit_ = dynamic_cast<TH1 *>(hm->getHisto(std::string(getHistoPrefix() + "nhit")));

  hist_nhit_south_ = dynamic_cast<TH1 *>(hm->getHisto(std::string(getHistoPrefix() + "nhit_south")));
  hist_nhit_north_ = dynamic_cast<TH1 *>(hm->getHisto(std::string(getHistoPrefix() + "nhit_north")));
  hist_pid_ = dynamic_cast<TH1 *>(hm->getHisto(std::string(getHistoPrefix() + "pid")));
  hist_adc_ = dynamic_cast<TH1 *>(hm->getHisto(std::string(getHistoPrefix() + "adc")));
  hist_bco_ = dynamic_cast<TH1 *>(hm->getHisto(std::string(getHistoPrefix() + "bco")));
  hist_bco_full_ = dynamic_cast<TH1 *>(hm->getHisto(std::string(getHistoPrefix() + "bco_full")));
...
std::string InttRawHitQA::getHistoPrefix() const
{

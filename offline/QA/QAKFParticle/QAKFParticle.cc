...
int QAKFParticle::process_event(PHCompositeNode *topNode)
{
  ...
  TH2F *h_mass_KFP_eta = dynamic_cast<TH2F *>(hm->getHisto(get_histo_prefix() + "InvMass_KFP_Eta"));
  assert(h_mass_KFP_eta);

  TH2F *h_mass_KFP_phi = dynamic_cast<TH2F *>(hm->getHisto(get_histo_prefix() + "InvMass_KFP_Phi"));
  assert(h_mass_KFP_phi);

  TH2F *h_mass_KFP_pt = dynamic_cast<TH2F *>(hm->getHisto(get_histo_prefix() + "InvMass_KFP_pT"));

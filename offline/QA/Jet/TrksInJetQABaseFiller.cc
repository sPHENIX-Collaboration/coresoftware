  ...
  , m_hist(hist)
{
  ...
  if (m_config.doHitQA)
  {
    m_hitManager = std::make_unique<TrksInJetQAHitManager>(m_config, m_hist);
  }
  if (m_config.doClustQA)
  {
    m_clustManager = std::make_unique<TrksInJetQAClustManager>(m_config, m_hist);
  }
  if (m_config.doTrackQA)
  {
    m_trackManager = std::make_unique<TrksInJetQATrkManager>(m_config, m_hist);
  }
  if (m_config.doJetQA)
  {
...
void TrksInJetQABaseFiller::SaveHistograms(TFile* outFile, const std::string& outDirName)
{
  ...
  if (m_config.doHitQA)
  {
    m_hitManager->SaveHistograms(outDir, m_config.hitOutDir);
  }
  if (m_config.doClustQA)
  {
    m_clustManager->SaveHistograms(outDir, m_config.clustOutDir);
  }
  if (m_config.doTrackQA)
  {
    m_trackManager->SaveHistograms(outDir, m_config.trackOutDir);
  }
  if (m_config.doJetQA)
  {
                                           ...
                                           std::vector<TH2D*>& vecOutHist2D)
{
  if (m_config.doHitQA)
  {
    m_hitManager->GrabHistograms(vecOutHist1D, vecOutHist2D);
  }
  if (m_config.doClustQA)
  {
    m_clustManager->GrabHistograms(vecOutHist1D, vecOutHist2D);
  }
  if (m_config.doTrackQA)
  {
    m_trackManager->GrabHistograms(vecOutHist1D, vecOutHist2D);
  }
  if (m_config.doJetQA)
  {
...
void TrksInJetQABaseFiller::GetNodes(PHCompositeNode* topNode)
{
  ...
  if (m_config.doJetQA)
  {
    m_jetMap = findNode::getClass<JetContainer>(topNode, m_config.jetInNode);
    if (!m_jetMap)
    {
  ...
  if (m_config.doTrackQA)
  {
    m_trkMap = findNode::getClass<SvtxTrackMap>(topNode, m_config.trkInNode);
    if (!m_trkMap)
    {
  ...
  if (m_config.doClustQA)
  {
    ...
    m_clustMap = findNode::getClass<TrkrClusterContainer>(topNode, m_config.clustInNode);
    if (!m_clustMap)
    {
  ...
  if (m_config.doHitQA)
  {
    m_hitMap = findNode::getClass<TrkrHitSetContainer>(topNode, m_config.hitInNode);
    if (!m_hitMap)
    {

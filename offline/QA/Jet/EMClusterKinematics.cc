...
EMClusterKinematics::EMClusterKinematics(const std::string &modulename, const std::string &inputnode, const std::string &histtag) :
  ...
  , m_doOptHist(false)
{
  if (Verbosity() > 1)
    {
      std::cout << "EMClusterKinematics::EMClusterKinematics(const std::string &name, const std::string&outputfilename) Calling ctor" << std::endl;
    }
}
//____________________________________________________________________________..
EMClusterKinematics::~EMClusterKinematics()
{
  if (Verbosity() > 1)
    {
      std::cout << "EMClusterKinematics::~EMClusterKinematics() Calling dtor" << std::endl;
    }
  delete m_analyzer;
}

//____________________________________________________________________________..
int EMClusterKinematics::Init(PHCompositeNode* /*topNode*/)
{
  if (Verbosity() > 1)
    {
      std::cout << "EMClusterKinematics::Init(PHCompositeNode *topNode) Initializing" << std::endl;
    }
...
//____________________________________________________________________________..
int EMClusterKinematics::InitRun(PHCompositeNode* /*topNode*/)
{

  if (Verbosity() > 1)
    {
      std::cout << "EMClusterKinematics::InitRun(PHCompositeNode *topNode) Initializing for Run XXX" << std::endl;
    }
  return Fun4AllReturnCodes::EVENT_OK;
}

//____________________________________________________________________________..
int EMClusterKinematics::process_event(PHCompositeNode *topNode)
{  

  RawClusterContainer* clusterContainer = findNode::getClass<RawClusterContainer>(topNode, m_inputnode);
  if (!clusterContainer)
    {
      std::cout << PHWHERE << "EMClusterKinematics::process_event - Fatal Error - CLUSTER_CEMC node is missing. " << std::endl;
      return 0;
    }
  ...
  if (Verbosity() > 1)
    {
      std::cout << "EMClusterKinematics::process_event(PHCompositeNode *topNode) Processing Event" << std::endl;
    }
  return Fun4AllReturnCodes::EVENT_OK;
}

//____________________________________________________________________________..
int EMClusterKinematics::ResetEvent(PHCompositeNode* /*topNode*/)
{
  if (Verbosity() > 1)
    {
      std::cout << "EMClusterKinematics::ResetEvent(PHCompositeNode *topNode) Resetting internal structures, prepare for next event" << std::endl;
    }
  return Fun4AllReturnCodes::EVENT_OK;
}

//____________________________________________________________________________..
int EMClusterKinematics::EndRun(const int runnumber)
{
  if (Verbosity() > 1) 
    {
      std::cout << "EMClusterKinematics::EndRun(const int runnumber) Ending Run for Run " << runnumber << std::endl;
    }
  return Fun4AllReturnCodes::EVENT_OK;
}

//____________________________________________________________________________..
int EMClusterKinematics::End(PHCompositeNode* /*topNode*/)
{
  // Commenting out the following line because Jenkins keeps failing the build test :( 
  // if (Verbosity() > 1) std::cout << "EMClusterKinematics::End - Output to " << outfilename << std::endl;
  ...
  if (Verbosity() > 1)
    {
      std::cout << "EMClusterKinematics::End(PHCompositeNode *topNode) This is the End..." << std::endl; 
    }
  return Fun4AllReturnCodes::EVENT_OK;
...
//____________________________________________________________________________..
int EMClusterKinematics::Reset(PHCompositeNode* /*topNode*/)
{
  if (Verbosity() > 1)
    {
      std::cout << "EMClusterKinematics::Reset(PHCompositeNode *topNode) being Reset" << std::endl;
    }
  return Fun4AllReturnCodes::EVENT_OK;
}

//____________________________________________________________________________..
void EMClusterKinematics::Print(const std::string &what) const
{
  if (Verbosity() > 1)
    {
      std::cout << "EMClusterKinematics::Print(const std::string &what) const Printing info for " << what << std::endl;
    }
}

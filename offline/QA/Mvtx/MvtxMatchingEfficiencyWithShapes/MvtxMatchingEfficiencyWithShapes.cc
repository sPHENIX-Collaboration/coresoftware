...
int MvtxMatchingEfficiencyWithShapes::InitRun(PHCompositeNode */*topNode*/)
{
  ...
  tree->Branch("shape_L0_C0_nhits", &nhits_arr[0][0], "shape_L0_C0_nhits/I");
  tree->Branch("shape_L0_C0_x",     &x_arr[0][0],     "shape_L0_C0_x/I");
  tree->Branch("shape_L0_C0_y",     &y_arr[0][0],     "shape_L0_C0_y/I");
  tree->Branch("shape_L0_C0_key",   &key_arr[0][0],   "shape_L0_C0_key/l");
  ...
  tree->Branch("shape_L1_C0_nhits", &nhits_arr[1][0], "shape_L1_C0_nhits/I");
  tree->Branch("shape_L1_C0_x",     &x_arr[1][0],     "shape_L1_C0_x/I");
  tree->Branch("shape_L1_C0_y",     &y_arr[1][0],     "shape_L1_C0_y/I");
  tree->Branch("shape_L1_C0_key",   &key_arr[1][0],   "shape_L1_C0_key/l");
  ...
  tree->Branch("shape_L2_C0_nhits", &nhits_arr[2][0], "shape_L2_C0_nhits/I");
  tree->Branch("shape_L2_C0_x",     &x_arr[2][0],     "shape_L2_C0_x/I");
  tree->Branch("shape_L2_C0_y",     &y_arr[2][0],     "shape_L2_C0_y/I");
  tree->Branch("shape_L2_C0_key",   &key_arr[2][0],   "shape_L2_C0_key/l");
...
int MvtxMatchingEfficiencyWithShapes::process_event(PHCompositeNode *topNode)
{
  ...
  if(!m_hitsetcontainer)std::cout<<"m_hitsetcontainer not found"<<std::endl;
  //assert(m_hitsetcontainer);

  // cluster hit association map
  m_cluster_hit_map = findNode::getClass<TrkrClusterHitAssoc>(topNode, "TRKR_CLUSTERHITASSOC");
  if(!m_cluster_hit_map)std::cout<<"m_cluster_hit_map not found"<<std::endl;
  ...
  for (auto it = trackmap->begin(); it != trackmap->end(); ++it) {
    ...
    if(good_intt_time == false){
      if(n_intt_hits ==2) h_INTT_time_delta->Fill(TMath::Abs(intt_time[0]-intt_time[1]));
      continue;
    } 
    intt_time.clear();
    h_status->Fill(2.5);
    ...
    for (int l = 0; l < nLayers; ++l){
      for (int c = 0; c < nClustersPerLayer; ++c){
        nhits_arr[l][c] = x_arr[l][c] = y_arr[l][c] = key_arr[l][c] = 0;
      }
    }
    ...
    for (const auto& ckey : TrackAnalysisUtils::get_cluster_keys(track)){
      switch (TrkrDefs::getTrkrId(ckey)){
        case TrkrDefs::mvtxId:{
          int layer = static_cast<int>(TrkrDefs::getLayer(ckey));
          ...
          int cL = 2000, cR = 0, cT = 2000, cB = 0;
          unsigned long long cmap = 0;
    ...
    //encode layer map
    layers = (mvtx_l[0] ? 1 : 0)    // L0 → bit 0
          | (mvtx_l[1] ? 2 : 0)     // L1 → bit 1
          | (mvtx_l[2] ? 4 : 0);    // L2 → bit 2

    //encode state map
    states = (mvtx_l_state[0] ? 1 : 0)    // L0 → bit 0
          | (mvtx_l_state[1] ? 2 : 0)     // L1 → bit 1
          | (mvtx_l_state[2] ? 4 : 0);    // L2 → bit 2
    tree->Fill();
  }

...
int CaloPacketv1::iValue(const int n,const std::string &what) const
{
  ...
  if ( what == "FEMSLOT")
  {
    if ( n < 0 || n >= getNrModules()) return 0;
    return femslot.at(n);
  }
...
void CaloPacketv1::dump(std::ostream &os) const
{
  ...
  os << "FEM Slot:    ";
  for ( int i = 0; i < iValue(0,"NRMODULES"); i++) os << std::setw(8) << iValue(i,"FEMSLOT");
  os <<  std::endl;

  os << "FEM Evt nr:  ";
  for ( int i = 0; i < iValue(0,"NRMODULES"); i++)  os << std::setw(8) << iValue(i,"FEMEVTNR");
  os << std::endl;

  os << "FEM Clock:   ";
  for ( int i = 0; i < iValue(0,"NRMODULES"); i++)  os << std::setw(8) << iValue(i,"FEMCLOCK");
  os << std::endl;

  os << "Even chksum: 0x" << std::hex << iValue(0,"EVENCHECKSUM")  << "   calculated:  0x" <<  iValue(0,"CALCEVENCHECKSUM");
  if ( iValue(0,"EVENCHECKSUMOK") == 1) os << " ok" << std::endl;
  else if ( iValue(0,"EVENCHECKSUMOK") == 0) os << " **wrong" << std::endl;

  os << "Odd chksum:  0x" << std::hex << iValue(0,"ODDCHECKSUM")  << "   calculated:  0x" <<  iValue(0,"CALCODDCHECKSUM");
  if ( iValue(0,"ODDCHECKSUMOK") == 1) os << " ok" << std::endl;
  else if ( iValue(0,"ODDCHECKSUMOK") == 0) os << " **wrong" << std::endl;
  os << std::dec << std::endl;

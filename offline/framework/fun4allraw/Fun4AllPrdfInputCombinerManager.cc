...
Fun4AllPrdfInputCombinerManager::~Fun4AllPrdfInputCombinerManager()
{
  if (IsOpen())
  {
    fileclose();
...
void Fun4AllPrdfInputCombinerManager::Print(const std::string &what) const
{
  Fun4AllInputManager::Print(what);
  return;
}

int Fun4AllPrdfInputCombinerManager::ResetEvent()
{
  PHNodeIterator iter(m_topNode);
  PHDataNode<Event> *PrdfNode = dynamic_cast<PHDataNode<Event> *>(iter.findFirst("PHDataNode", m_PrdfNodeName));
...
int Fun4AllPrdfInputCombinerManager::PushBackEvents(const int i)
{
  ...
  //        << " Fun4AllPrdfInputCombinerManager cannot push back " << i << " events into file"
  //        << std::endl;
  //   return -1;
  // }
  // if (!m_EventIterator)
...
int Fun4AllPrdfInputCombinerManager::GetSyncObject(SyncObject **mastersync)
{
...
int Fun4AllPrdfInputCombinerManager::SyncIt(const SyncObject *mastersync)
{
  if (!mastersync)
  {
...
std::string Fun4AllPrdfInputCombinerManager::GetString(const std::string &what) const
{
...
SinglePrdfInput *Fun4AllPrdfInputCombinerManager::AddPrdfInputFile(const std::string &filenam)
{
  FROG frog;
...
void Fun4AllPrdfInputCombinerManager::UpdateEventFoundCounter(const int evtno)
{

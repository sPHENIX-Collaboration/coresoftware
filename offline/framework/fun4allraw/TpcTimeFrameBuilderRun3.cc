  ...
  , m_HistoPrefix("TpcTimeFrameBuilderRun3_Packet" + std::to_string(packet_id))
{
  ...
  hm->registerHisto(h_Run3WaveformStart_GL1Spacing);

  h_Run3PreviousTimeFrameWaveformStart = new TH1I(TString(m_HistoPrefix.c_str()) + "_Run3PreviousTimeFrameWaveformStartCache",  //
                                                  TString(m_HistoPrefix.c_str()) +
                                                      " Run3 previous matched waveform start cache;FEE ADC waveform start clock;Count",
                                                  1024, -.5, 1023.5);
  h_Run3PreviousTimeFrameWaveformStart->SetDirectory(nullptr);
...
void TpcTimeFrameBuilderRun3::flush_previous_timeframe_waveform_start_cache(uint64_t current_gtm_bco)
{
  assert(h_Run3PreviousTimeFrameWaveformStart);
  assert(h_Run3WaveformStart_GL1Spacing);
  ...
  for (int bin = 1; bin <= h_Run3PreviousTimeFrameWaveformStart->GetNbinsX(); ++bin)
  {
    const double count = h_Run3PreviousTimeFrameWaveformStart->GetBinContent(bin);
    if (count == 0)
    {
      continue;
    }

    h_Run3WaveformStart_GL1Spacing->Fill(h_Run3PreviousTimeFrameWaveformStart->GetXaxis()->GetBinCenter(bin),
                                         gtm_bco_spacing, count);
  }
...
std::vector<TpcRawHit*>& TpcTimeFrameBuilderRun3::getTimeFrame(const uint64_t& gtm_bco)
{
  ...
  flush_previous_timeframe_waveform_start_cache(bclk_rollover_corrected);
...
int TpcTimeFrameBuilderRun3::ProcessPacket(Packet* packet)
{
  ...
  if (m_hitFormat < 0)
  {
    ...
    if (m_verbosity >= 1)
    {
      std::cout << __PRETTY_FUNCTION__ << " set exact Run3 clock sync for hit format " << m_hitFormat
                << " with clock ratio = 30/8" << std::endl;
    }
    for (BcoMatchingInformation& bcoMatchingInformation : m_bcoMatchingInformation_vec)
    {
      bcoMatchingInformation.set_gtm_clock_ratio(30, 8);
    }
  }
...
void TpcTimeFrameBuilderRun3::process_fee_data_waveform(const unsigned int& fee, std::deque<uint16_t>& data_buffer)
{
  ...
  if ((!m_fastBCOSkip) && payload.has_clock_sync)
  {
    ...
    if (payload.type != TpcTimeFrameBuilderRun3::BcoMatchingInformation::HEARTBEAT_T)
    {
      ...
      waveform_start_clocks.reserve(payload.waveforms.size());
      for (const std::pair<uint16_t, std::vector<uint16_t>>& waveform : payload.waveforms)
      {
...
bool TpcTimeFrameBuilderRun3::BcoMatchingInformation::isMoreDataRequired(const uint64_t& gtm_bco) const
{
  ...
  if (!m_bco_reference_candidate_list.empty())
  {
    if (m_bco_reference_candidate_list.back().first > bco_correction + m_max_fee_sync_time)
    {
      if (m_verbosity >= 2)
      {
        std::cout << "TpcTimeFrameBuilderRun3[" << m_name << "]::BcoMatchingInformation::isMoreDataRequired"
                  << "at gtm_bco = 0x" << std::hex << gtm_bco << std::dec
                  << ". m_bco_reference_candidate_list.back().first = 0x" << std::hex << m_bco_reference_candidate_list.back().first << std::dec
                  << " bco_correction = 0x" << std::hex << bco_correction << std::dec
                  << ". satisified m_max_fee_sync_time = " << m_max_fee_sync_time
                  << std::endl;
      }
...
std::optional<uint64_t> TpcTimeFrameBuilderRun3::BcoMatchingInformation::find_reference_heartbeat(const TpcTimeFrameBuilderRun3::fee_payload& HeartBeatPacket)
{
  ...
  if (m_bco_reference)
  {
    ...
    if (get_fee_bco_diff(fee_bco_predicted, fee_bco) < m_max_fee_bco_diff)
    {
      ...
      if (verbosity() > 1)
      {
        std::cout << "TpcTimeFrameBuilderRun3[" << m_name << "]::BcoMatchingInformation::find_reference_heartbeat - found an updated reference heartbeat and updated reference clock sync: "
                  << std::hex
                  << "\t- fee_bco: 0x" << fee_bco
  ...
  for (const m_gtm_fee_bco_matching_pair_t& bco : m_bco_reference_candidate_list)
  {
    ...
    if (get_fee_bco_diff(fee_bco_predicted, fee_bco) < m_max_fee_bco_diff)
    {
      if (verbosity() > 1)
      {
        std::cout << "TpcTimeFrameBuilderRun3[" << m_name << "]::BcoMatchingInformation::find_reference_heartbeat - found a new reference canidate heartbeat and replaced reference clock sync: "
                  << std::hex
                  << "\t- fee_bco: 0x" << fee_bco
                  << "\t- predicted: 0x" << fee_bco_predicted
                  << "\t- gtm_bco: 0x" << gtm_bco
      ...
      // assign gtm bco
      m_bco_reference = std::make_pair(gtm_bco, fee_bco);
      ...
      if (m_verbosity > 1)
      {
        ...
        {
          std::cout << "\t\t- gtm_bco = 0x" << std::hex << bco_tmp.first << std::dec
                    << "\t\t- fee_bco = 0x" << std::hex << bco_tmp.second << std::dec
                    << std::endl;
        }
      }

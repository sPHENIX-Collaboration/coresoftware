...
private:
    ...
    uint16_t m_EMC_skim_threshold{192}; // skim if nchannels >= this many not-instrumented(empty/missing pckt) channels in EMCal

    bool b_do_skim_HCal{false};
    uint16_t m_HCal_skim_threshold{192}; // skim if nchannels >= this many not-instrumented(empty/missing pckt) channels in HCal

    bool b_do_skim_sEPD{false};
    uint16_t m_sEPD_skim_threshold{192}; // skim if nchannels >= this many not-instrumented(empty/missing pckt) channels in sEPD

    bool b_do_skim_ZDC{false};
    uint16_t m_ZDC_skim_threshold{192}; // skim if nchannels >= this many not-instrumented(empty/missing pckt) channels in ZDC
};

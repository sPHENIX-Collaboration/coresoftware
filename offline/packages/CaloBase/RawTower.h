...
class RawTower : public PHObject
{
 public:
  ...
  void identify(std::ostream& = std::cout) const override { PHOOL_VIRTUAL_WARN("identify()"); }
  ...
  virtual double get_property(const PROPERTY) const { return NAN; }
  virtual void set_property(const PROPERTY, const double) { return; }

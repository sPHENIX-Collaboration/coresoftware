...
Acts::Vector3 ActsGeometry::getGlobalPositionTpc(TrkrDefs:: cluskey key,	 TrkrCluster* cluster)
{
  ...
  // We really just need the local to global to get the rphi position, since z is
  // undetermined in streaming mode
  glob = surface->localToGlobal(geometry().getGeoContext(),
				  local * Acts::UnitConstants::cm,
				  Acts::Vector3(1,1,1));
  glob /= Acts::UnitConstants::cm;
  std::cout << "Global is " << glob.transpose() << std::endl;
  // Reset z based on streaming mode
  if(_extended_readout_time > 0)
  {
    std::cout << "_tdriftmax " << _tdriftmax <<std::endl;
  ...
  std::cout << "Actual global is " << glob.transpose() << std::endl;
  return glob;
}

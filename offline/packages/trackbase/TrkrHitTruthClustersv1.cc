...
TrkrHitTruthClustersv1::print_clusters(std::ostream &os) const
{
  ...
  for( const auto& entry : m_map )
  {
    ...
    for (unsigned int i_cluster=0; i_cluster<entry.second.size(); ++i_cluster) {
        ...
        for (int k{0}; k<6; ++k) os << " " << std::setw(10) << entry.second[i_cluster][k];
        os << std::endl;
    }
    os << std::endl;
  }
        ...
        const std::array<double,8>& phiRz_data, const double sum_E)
{
    ...
    if (print_local) {
        ...
        for (auto& v : phiRz_data) std::cout << " " << v;
    ...
    for (int i : std::vector<int>{0,2,4,6}) {
        ...
        if (print_local) std::cout << " Added: ("<<i<<") : " << phiRz_calc[i] << " " << phiRz_calc[i+1] << std::endl;
    }
    std::array<float,6> phiRz_entry {};
    for (int i : std::vector<int>{2,3,4,5}) phiRz_entry[i] = phiRz_calc[i];
    ...
    if (m_map.find(i_track) != m_map.end()) m_map[i_track].push_back(phiRz_entry);
    else m_map[i_track] = {phiRz_entry};

    if (print_local) for (auto& v_pairs : m_map) {

...
void TpcTpotEventInfov1::identify(std::ostream& os) const
{
  ...
  for (int i = 0; i < 25; i++)
  {
    ...
    os << "Event - " << m_event[i][0][0] << ", " << m_event[i][0][1] << ", " << m_event[i][1][0] << ", " << m_event[i][1][1] << std::endl;
  ...
  for (int i = 0; i < 25; i++)
  {
    for (int j = 0; j < 2; j++)
    {
      for (int k = 0; k < 2; k++)
      {
        ...
        setEvent(source.getEvent(static_cast<SectorID>(i), static_cast<PCIeEndPointID>(j), static_cast<TaggerID>(k)),static_cast<SectorID>(i), static_cast<PCIeEndPointID>(j), static_cast<TaggerID>(k));

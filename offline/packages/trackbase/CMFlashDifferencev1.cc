...
{
  if (m_nclusters == UINT_MAX)
  {
    return 0;
  }

  if (std::isnan(getTruthPhi()))
  {
    return 0;
  }
  if (std::isnan(getTruthR()))
  {
    return 0;
  }
  if (std::isnan(getTruthZ()))
  {
    return 0;
  }

  if (std::isnan(getRecoPhi()))
  {
    return 0;
  }
  if (std::isnan(getRecoR()))
  {
    return 0;
  }
  if (std::isnan(getRecoZ()))
  {
    return 0;
  }

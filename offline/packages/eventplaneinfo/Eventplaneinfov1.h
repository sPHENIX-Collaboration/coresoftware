...
class Eventplaneinfov1 : public Eventplaneinfo
{
 public:
  ...
  std::pair<double, double> get_qvector(unsigned int order) const override { return std::make_pair(mQvec[order - 1].first, mQvec[order - 1].second); }
  void set_ring_qvector(const std::vector<std::vector<std::pair<double, double>>>& Qvec) override { ring_Qvec = Qvec; }
  std::pair<double, double> get_ring_qvector(int ring_index, unsigned int order) const override { return ring_Qvec[ring_index][order - 1]; }
  double get_ring_psi(int ring_index, unsigned int order) const override {return GetPsi(ring_Qvec[ring_index][order - 1].first,ring_Qvec[ring_index][order - 1].second,order);}
  double GetPsi(double Qx, double Qy, unsigned int order) const override;
  double get_psi(unsigned int order) const override { return GetPsi(mQvec[order - 1].first, mQvec[order - 1].second, order);}
  double get_shifted_psi(unsigned int order) const override { return mPsi_Shifted[order - 1]; }

...
namespace
{
                                ...
                                const bool use_straight_line)
  {
    ...
    for (const Tpc_PolyCluster* cluster : clusters)
    {
      ...
      double x = 0.0;
      double y = 0.0;
      if (!poly_track_xy_at_z(poly_track, cz, magnetic_field_tesla, arc_direction, use_straight_line, x, y))
      {
        continue;
      }
                                    ...
                                    const int color)
  {
    ...
    for (unsigned int istep = 0; istep <= nsteps; ++istep)
    {
      ...
      double x = 0.0;
      double y = 0.0;
      if (!poly_track_xy_at_z(trk, z, magnetic_field_tesla, arc_direction, use_straight_line, x, y))
      {
        continue;
      }
...
int Tpc_PolyClusterDisplay::Init(PHCompositeNode* /*unused*/)
{
...
int Tpc_PolyClusterDisplay::process_event(PHCompositeNode* topNode)
{
  ...
  for (unsigned int ifinal = 0; ifinal < npoly_tracks; ++ifinal)
  {
    ...
    if (!cluster_group_z_range(cluster_iter->second, m_zmin, m_zmax, line_zmin, line_zmax))
    {
      continue;
    }
  ...
  for (unsigned int ivtx = 0; ivtx < nvertices; ++ivtx)
  {
    ...
    if (!track_vertex_z_selected(vtx, m_trackVertexZMin, m_trackVertexZMax))
    {
      continue;
    }
  ...
  for (unsigned int ivtx = 0; ivtx < ncollision_vertices; ++ivtx)
  {
    ...
    if (marker)
    {
      collision_vertex_markers.push_back(marker);
    }
  }
...
int Tpc_PolyClusterDisplay::End(PHCompositeNode* /*unused*/)
{
  if (m_outfile)
  {
    m_outfile->Close();

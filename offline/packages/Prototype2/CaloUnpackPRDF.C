...
int
CaloUnpackPRDF::GetHBDCh(string caloname, int i_column, int i_row)
{
  if (caloname == "HCALIN")
    {
  ...
  else if (caloname == "EMCAL")
    {
      ...
      const int tower_index = i_column + 8 * (7 - i_row);

      assert(tower_index >= 0);
      assert(tower_index < 64);

      return canmap[tower_index];
    }

  cout << "CaloUnpackPRDF::GetHBDCh - invalid input caloname " << caloname
      << " i_column " << i_column << " i_row " << i_row << endl;
  exit(1);
  return -9999;
}

...
void EpFinderReco::GetEventPlanes(PHCompositeNode *topNode) {
  ...
  *_CEMCHCAL_EpInfo = CEMCHCAL_EpResult;
    ...
    for(PHG4HitContainer::ConstIterator b_itr = b_range.first; b_itr != b_range.second; b_itr++)
    {
      ...
      EpInfo nb_EpResult = EpFinder_1->Results(&nbhits,0);
      *_BBC_EpInfoN = nb_EpResult;

      EpInfo sb_EpResult = EpFinder_2->Results(&sbhits,0);
      *_BBC_EpInfoS = sb_EpResult;
    ...
    for(PHG4HitContainer::ConstIterator e_itr = e_range.first; e_itr != e_range.second; e_itr++)
    {
        ...
        if(ehit->get_light_yield()>0.0)
        {
            ...
            {
                phi_bin = GetPhiBin(ehitPos.Phi(), num_phi/2);
                eta_bin = heta->FindBin(ehitPos.Eta()) - 1;
            }
            }
      ...
      for(int i=0; i<num_eta; i++){
         for(int j=0; j<num_phi; j++){
             if(_epd_e[1][i][j]>0.0){
                ...
                if(i==15) meanPhi = GetMeanPhi(j, num_phi/2);
...
hcalotowers = findNode::getClass<RawTowerContainer>(topNode, "TOWER_CALIB_HCALOUT");
 ...
 { 
  std::cout << PHWHERE
            << " Detector choice does not mactch, use BBC for this mode, exiting"
            << std::endl;
     gSystem->Exit(1);
     exit(1);
 }
 ...
 { 
  std::cout << PHWHERE
            << " Detector choice does not mactch, use EPD for this mode, exiting"
            << std::endl;
     gSystem->Exit(1);
     exit(1);
 }  

...
TrkrHitSetContainer::Iterator hitsetit = m_hitsetcontainer->findOrAddHitSet(hitsetkey);
for ( int s = 0; s < p->iValue(wf, "SAMPLES"); s++)
{
TrkrDefs::hitkey hitkey = TcpDefs::genHitKey(pad, s + 2*( current_BCO - starting_BCO) );

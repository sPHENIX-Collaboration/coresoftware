...
int ParticleFlowReco::process_event(PHCompositeNode *topNode)
{
  ...
  for (unsigned int had = 0; had < _pflow_HAD_E.size(); had++)
  {
    ...
    else
    {
      ...
      TLorentzVector tlv;
      tlv.SetPtEtaPhiM(residual_energy / cosh(_pflow_HAD_eta[had]), _pflow_HAD_eta[had], _pflow_HAD_phi[had], 0);
  ...
  for (unsigned int em = 0; em < _pflow_EM_E.size(); em++)
  {
    ...
    else
    {
      ...
      // assume no mass, but could update to use K0L mass(?)
      TLorentzVector tlv;
      tlv.SetPtEtaPhiM(residual_energy / cosh(_pflow_EM_eta[em]), _pflow_EM_eta[em], _pflow_EM_phi[em], 0);
  ...
  for (unsigned int em = 0; em < _pflow_EM_E.size(); em++)
  {
    ...
    TLorentzVector tlv;
    tlv.SetPtEtaPhiM(_pflow_EM_E[em] / cosh(_pflow_EM_eta[em]), _pflow_EM_eta[em], _pflow_EM_phi[em], 0);
  ...
  for (unsigned int had = 0; had < _pflow_HAD_E.size(); had++)
  {
    ...
    // assume massless, could be updated to use K0L
    TLorentzVector tlv;
    tlv.SetPtEtaPhiM(_pflow_HAD_E[had] / cosh(_pflow_HAD_eta[had]), _pflow_HAD_eta[had], _pflow_HAD_phi[had], 0);

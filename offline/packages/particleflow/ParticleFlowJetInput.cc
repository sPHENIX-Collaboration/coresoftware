...
std::vector<Jet *> ParticleFlowJetInput::get_input(PHCompositeNode *topNode)
{
  if (_verbosity > 0)
  {
    std::cout << "ParticleFlowJetInput::process_event -- entered" << std::endl;
  }
  ...
  for (rtiter = begin_end.first; rtiter != begin_end.second; ++rtiter)
  {
    ...
    if (_verbosity > 15)
    {
      for (auto c : pseudojets[pseudojets.size() - 1]->get_comp_vec())
      {
        std::cout << " GOT " << ((int) c.first) << " and " << ((int) c.second) << std::endl;
      }
    }
  }
  ...
  if (_verbosity > 0)
  {
    std::cout << "ParticleFlowJetInput::process_event -- exited" << std::endl;
  }

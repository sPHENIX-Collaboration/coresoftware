...
namespace 
{
  ...
  struct thread_data 
  {
    ...
    clus->setAdc(adc_sum);
	  ...
	  Acts::Vector3 center = surface->center(my_data.tGeometry->m_tGeometry.geoContext) 
	    / Acts::UnitConstants::cm;
	  
	  /// no conversion needed, only used in acts
	  const Acts::Vector3 normal = surface->normal(my_data.tGeometry->m_tGeometry.geoContext);

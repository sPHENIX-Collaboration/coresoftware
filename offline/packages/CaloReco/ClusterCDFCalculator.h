...
private:
    TFile* file{nullptr};
    TH1D* henbins{nullptr};

    std::vector<TH1D*> hD2_3x3;
    std::vector<TH1D*> hD2_5x5;
    std::vector<TH1D*> hD2_7x7;

    std::vector<TMatrixD> inverseCovarianceMatrix_3x3;
    std::vector<TMatrixD> inverseCovarianceMatrix_5x5;
    std::vector<TMatrixD> inverseCovarianceMatrix_7x7;

    std::vector<double> meanD2_3x3;
    std::vector<double> meanD2_5x5;
    std::vector<double> meanD2_7x7;

    std::vector<std::vector<TH1D*>> ratioHistograms_3x3;
    std::vector<std::vector<TH1D*>> ratioHistograms_5x5;
    std::vector<std::vector<TH1D*>> ratioHistograms_7x7;
...
public:
    ...
    double GetCDF(const std::vector<double>& energies, double clusterenergy, int NMATRIX);
};

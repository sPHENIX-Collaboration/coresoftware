...
float BEmcProfile::GetProb(std::vector<EmcModule>* plist, int NX, float en, float theta)
{
  ...
  for( int ip=0; ip<NP; ip++ ) {
    ...
    if( ep[ip]<0 ) return -1;
    if( ip<3 ) err[ip] = sqrt(err[ip]*err[ip]+4*enoise*enoise/etot/etot);
    else       err[ip] = sqrt(err[ip]*err[ip]+1*enoise*enoise/etot/etot);
  }

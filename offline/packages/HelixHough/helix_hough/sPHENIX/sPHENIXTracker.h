...
{
public:
  TrackSegment() : chi2(0.), ux(0.), uy(0.), kappa(0.), dkappa(0.), seed(0), n_hits(0) {}
  ~TrackSegment(){}
...
//     required_layers = min_hits;
  ...
  void findSeededTracksbySegments_run(std::vector<SimpleTrack3D>& seeds, std::vector<SimpleHit3D>& hits, std::vector<SimpleTrack3D>& tracks);
  ...
  {
    if(layer >= hit_error_scale.size()){hit_error_scale.resize( layer+1, 1. );}

        ...
        for(unsigned int th=0;th<n_dzdl;++th)
        {
          for(unsigned int zz0=0;zz0<n_z0;++zz0)
          {
            ...
            if(ca > 0)
            {
              ...
              if(attemptClusterMerge(zoomlevel, MAX, ca, d, r, th, zz0, bin, newbin, good_bins, volume, cluster_size_cut, overlap_cut, clusters, (unsigned int*)(bins_start), (unsigned int*)(bins_end), map_clus, too_big, temp_merged_clus, C_clus) == true){continue;}
            }
...
void HelixHough::findSeededHelices(vector<SimpleTrack3D>& seeds, vector<SimpleHit3D>& hits, unsigned int min_hits, unsigned int max_hits, vector<SimpleTrack3D>& tracks, unsigned int maxtracks)
{
  ...
  for(unsigned int i=0;i<hits.size();++i)
  {
    if(hits[i].layer > max_layer){max_layer = hits[i].layer;}
    if(hits[i].layer < min_layer){min_layer = hits[i].layer;}
    if(hits[i].layer < 0)
    {
  ...
  while(layers_left > 0)
  {
    ...
    for(unsigned int i=0;i<hits.size();++i)
    {
      if( (hits[i].layer >= cur_layer) && (hits[i].layer <= (cur_layer+layers_at_a_time-1)) )
      {
    ...
    if(layers_left <= layers_at_a_time){smooth_back = smooth_back_orig;}
    
    findSeededHelices_run(cur_seeds, cur_hits, nreq, max_hits, cur_tracks, maxtracks);
...
void HelixHough::findSeededHelices_run(vector<SimpleTrack3D>& seeds, vector<SimpleHit3D>& hits, unsigned int min_hits, unsigned int max_hits, vector<SimpleTrack3D>& tracks, unsigned int maxtracks)
{
  vote_time = 0.;
  xy_vote_time = 0.;
  z_vote_time = 0.;

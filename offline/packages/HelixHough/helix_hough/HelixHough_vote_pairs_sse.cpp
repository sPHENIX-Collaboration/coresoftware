    ...
    for (unsigned int i = 0; i < pairs_vec[zoomlevel]->size(); i++) {
      ...
      if (pair_counter == 4) {
        ...
        for (unsigned int h = 0; h < pair_counter; ++h) {
          float dr0 = sqrt(four_pairs[h][0].dx * four_pairs[h][0].dx +
                           four_pairs[h][0].dy * four_pairs[h][0].dy);
          float r0 = sqrt(four_pairs[h][0].get_x() * four_pairs[h][0].get_x() +
                          four_pairs[h][0].get_y() * four_pairs[h][0].get_y());
          float dr1 = sqrt(four_pairs[h][1].dx * four_pairs[h][1].dx +
                           four_pairs[h][1].dy * four_pairs[h][1].dy);
          ...
          float dz0 =
              r0 * (four_pairs[h][0].dz + four_pairs[h][1].dz) * r1r0_inv;
          dz0 += dzdl_scatt * r1;
          dz0 *= error_scale;
          min_z0_1_a[h] -= dz0;
          min_z0_2_a[h] -= dz0;

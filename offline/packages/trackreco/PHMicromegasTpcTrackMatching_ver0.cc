...
int PHMicromegasTpcTrackMatching::process_event(PHCompositeNode* topNode)
{
       ...
       seedID != _svtx_seed_map->size(); ++seedID)
  {
    ...
    for (unsigned int imm = 0; imm < _n_mm_layers; ++imm)
    {
      ...
      bool first = true;
      int good_track_counter = 0;
      int bad_track_counter = 0;
	  ...
	  if (std::abs(drphi) < 1.0 && std::abs(dz) < 1.0 && good_track_counter < 10)
          {

            std::cout << "GOOD TRACK " << good_track_counter << std::endl;
	    std::cout << "MM cluster global: x " << glob.x()
		    << " y " << glob.y()
		    << " z " << glob.z() << std::endl;

	    const auto& cluster_set = _use_silicon ? clusGlobPos_silicon : clusGlobPos;
	    for (size_t i = 0; i < cluster_set.size(); ++i)
	    {

              const auto& pos = cluster_set[i];
	      std::cout << "Cluster[" << i << "]: x " << pos.x()
		      << " y " << pos.y()
		      << " z " << pos.z() << std::endl;
	    }
	    std::cout << "Projection (planar): x " << world_intersection_planar.x()
		    << " y " << world_intersection_planar.y()
		    << " z " << world_intersection_planar.z() << std::endl;
	    std::cout << "slope_xy " << slope_xy << " intercept_xy " << intersect_xy << std::endl;
	    std::cout << "slope_rz " << slope_rz << " intercept_rz " << intersect_rz << std::endl;

	    ++good_track_counter;

	  } // good tracks
	  else if (std::abs(drphi) > 1.0 && std::abs(dz) > 1.0 && bad_track_counter < 10)
	  {
	    std::cout << "BAD TRACK " << bad_track_counter << std::endl;
	    std::cout << "MM cluster global: x " << glob.x()
		    << " y " << glob.y()
		    << " z " << glob.z() << std::endl;

...
int PHTpcClusterMover::process_event(PHCompositeNode */*topNode*/)
{
  ...
  for( const auto& [cluskey, global]:tpc_clusters )
  {
    ...
    Acts::Vector3 normal = surface->normal(_tGeometry->m_tGeometry.geoContext);
    auto local = surface->globalToLocal(_tGeometry->m_tGeometry.geoContext,
	  ...
	  if(local.ok())
	  ...
	  else
	    {
	      ...
	      Acts::Vector3 center = surface->center(_tGeometry->m_tGeometry.geoContext)/Acts::UnitConstants::cm;

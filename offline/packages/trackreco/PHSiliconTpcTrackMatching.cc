...
int PHSiliconTpcTrackMatching::process_event(PHCompositeNode*)
{
  ...
  if(Verbosity() > 2)
	   ...
	   ++phtrk_iter_si)
	{
	  ...
	  cout << " Si track " << _track_map_silicon->index(phtrk_iter_si) << " si_phi " << si_phi  << " si_eta " << si_eta << endl;
  ...
  // now we need the crossing values
  ...
  if(_pp_mode)
      ...
      if(_use_intt_time)
	{
	  ...
	  getMatchCrossingIntt(tpc_matches, crossing_matches, tpc_crossing_map);
	...
	{
	  // Use the z-mismatch between the silicon and TPC tracklets to assign the crossing
  ...
  for(auto it = crossing_matches.begin(); it != crossing_matches.end(); ++it)
    {
      ...
      TrackSeed *si_track = _track_map_silicon->get(si_id);
      if(Verbosity() > 1) std::cout << "  si track id " << si_id << std::endl;
      for (TrackSeed::ConstClusterKeyIter si_iter = si_track->begin_cluster_keys();
	   si_iter != si_track->end_cluster_keys();
	   ++si_iter)
	{
	  TrkrDefs::cluskey si_cluster_key = *si_iter;

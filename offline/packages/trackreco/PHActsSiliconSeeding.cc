  ...
  for (const auto& seeds : seedVector)
  {
    ...
    for (const auto& seed : seeds)
    {
      ...
      if (m_searchInIntt)
      {
        ...
        /// if acts found more than 3 clusters in the INTT only it is likely a bad seed (overlaps exist between layers 3 & 4, and 5 & 6)
        if (nintt > 3)
        {
          continue;
        }
    ...
    TrkrCluster* clus)
{
  ...
  float x = globalPos.x() - v_globaldisplacement[0];
  float y = globalPos.y() - v_globaldisplacement[1];
  float z = globalPos.z() - v_globaldisplacement[2];
  float r = std::sqrt(x * - + y * y);

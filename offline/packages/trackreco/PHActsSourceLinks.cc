...
int PHActsSourceLinks::process_event(PHCompositeNode *topNode)
{
  ...
  for(clusIter = clusRange.first; clusIter != clusRange.second; ++clusIter)
      ...
      for(int i=0; i < 3; ++i)
	{
      ...
      /// Obtain the hitsetkey from the cluskey for the specific detector
      ...
      else if (trkrId == TrkrDefs::tpcId)
	{
	  ...
	  std::map<TrkrDefs::cluskey, 
		   std::shared_ptr<const Acts::Surface>>::iterator surfIter;
	  ...
	  surface = surfIter->second->getSharedPtr();
...
/**
 * Get all the nodes necessary for processing off the node tree
 */
int PHActsSourceLinks::GetNodes(PHCompositeNode *topNode)
{
  ...
  m_clusterSurfaceMap = findNode::getClass<std::map<TrkrDefs::hitsetkey, std::shared_ptr<const Acts::Surface>>>(topNode, "clusterSurfaceMapName");
...
/**
 * Create nodes on the node tree that this module will populate 
 */
...
std::shared_ptr<const Acts::Surface> PHActsSourceLinks::getSurfaceFromClusterMap(TrkrDefs::hitsetkey hitSetKey)
{
  std::shared_ptr<const Acts::Surface> surface;

  std::map<TrkrDefs::hitsetkey, 
	   std::shared_ptr<const Acts::Surface>>::iterator 
    surfaceIter;

...
Surface ActsEvaluator::getSiliconSurface(TrkrDefs::hitsetkey hitsetkey)
{
  ...
  auto surfMap = m_surfMaps->getSiliconMap();
...
Surface ActsEvaluator::getTpcSurface(TrkrDefs::hitsetkey hitsetkey, TrkrDefs::subsurfkey surfkey)
{
  ...
  const auto iter = m_surfMaps->getTpcMap().find(layer);
  if(iter != m_surfMaps->getTpcMap().end())
  {
...
{
  const auto iter = m_surfMaps->getMMMap().find( hitsetkey );
  return (iter == m_surfMaps->getMMMap().end()) ? nullptr:iter->second;
}

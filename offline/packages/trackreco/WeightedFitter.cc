    ...
    double const* params) const
{
  ...
  double cos_theta = std::cos(params[2]);
  double sin_theta = std::sin(params[2]);
  double cos_phi = std::cos(params[3]);
  double sin_phi = std::sin(params[3]);
...
int WeightedFitter::process_event(
    PHCompositeNode* top_node)
{
  try
  {
  ...
  m_track_id = 0;
  for (auto& track_seed_itr : *m_track_seed_container)
  {
    if (!track_seed_itr)
    {
      continue;
...
bool WeightedFitter::get_points(
    TrackSeed const* track_seed)
{
      ...
      ++cluster_key_itr)
  {
    ...
    if (1.0 < cluster->getRPhiError())
    {
      continue;
    }
    if (1.0 < cluster->getZError())
    {
      continue;
    }
  ...
  if (n_mvtx < m_num_mvtx)
  {
    return true;
  }
  if (n_intt < m_num_intt)
  {
    return true;
  }
  if (n_tpc < m_num_tpc)
...
void WeightedFitter::modify_point_tpc(
    ...
    WeightedFitter::ClusterFitPoint& point)
{
  ...
  point.pos.z() = TpcClusterZCrossingCorrection::correctZ(point.pos.z(), side, 0);
...
void WeightedFitter::draw_fit_xy(
    TPad* pad)
{
  double x_min = -120;
  double x_max = 120;
...
void WeightedFitter::draw_cluster_xy(
    TPad* pad,

...
/*
bool SimpleHit3D_LessThan(SimpleHit3D i, SimpleHit3D j)
{
  return (i.index<j.index);
}

bool SimpleTrack3D_Equality(SimpleTrack3D i, SimpleTrack3D j)
{
  if(i.hits.size()!=j.hits.size())
    return false;
  for(unsigned int ihit = 0; ihit < i.hits.size(); ihit++)
    if(i.hits[ihit].index!=j.hits[ihit].index)
      return false;
    return true;
}

bool is_lessthan(vector<SimpleHit3D> i, vector<SimpleHit3D> j, unsigned int ihit)
{
  if(i[ihit].index<j[ihit].index){//if any index for i is < j then it is less than
    return true;
  }
  else if(i[ihit].index==j[ihit].index){//if equal
    if(ihit < i.size()-1)//and not at the end of the vector
      return is_lessthan(i,j,ihit+1);//look at the next index
      else return false;//otherwise at end of vector being equal so return false
  }
  else return false;//if index is not < or == then it must be > so return false
}

bool SimpleTrack3D_LessThan(SimpleTrack3D i , SimpleTrack3D j)
{//wrapper for recursive is_lessthan

if(i.hits.size()!=j.hits.size()){//if the hits aren't the same size
    if(i.hits.size()<j.hits.size())//and if no. of hits in i < no. of hits in j
      return true; //consider i < j
      else return false; // otherwise i > j
}
return is_lessthan(i.hits,j.hits,0);//i.size should == j.size so we should be able to compare them index by index

}
*/

VtxTrackFinder::VtxTrackFinder(vector<float>& detrad, unsigned int n_phi, unsigned int n_d, unsigned int n_k, unsigned int n_dzdl, unsigned int n_z0, HelixResolution& min_resolution, HelixResolution& max_resolution, HelixRange& range) : HelixHough(n_phi, n_d, n_k, n_dzdl, n_z0, min_resolution, max_resolution, range), using_vertex(false), vertex_sigma_xy(0.002), vertex_sigma_z(0.005), chi2_cut(3.)
{
...
void VtxTrackFinder::findTracks(vector<SimpleHit3D>& hits, vector<SimpleTrack3D>& tracks)
{
...
void VtxTrackFinder::finalize(vector<SimpleTrack3D>& input, vector<SimpleTrack3D>& output)
{
...
double VtxTrackFinder::fitTrack(SimpleTrack3D& track, vector<double>& chi2_hit)
{

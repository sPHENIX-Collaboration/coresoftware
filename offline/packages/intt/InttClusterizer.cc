/**
 * @file intt/InttClusterizer.cc
 * @author D. McGlinchey
 * @date June 2018
 * @brief Implementation of InttClusterizer
 */
...
bool InttClusterizer::are_adjacent(const pixel lhs,
                                   const pixel rhs )
{
...
void InttClusterizer::ClusterIntt(PHCompositeNode *topNode) {
  
  if (verbosity > 0)
    cout << "Entering InttClusterizer::ClusterIntt " << endl;
  
  //-----------
  // Clustering
  //-----------

  // loop over each InttHitSet object (chip)
        ...
        ++hitsetitr )
  {
          ...
          ++hitr)
    {
      ...
      // (we don't actually care about the hit itself for the intt)
      unsigned int col = InttDefs::getCol(hitr->first);
      unsigned int row = InttDefs::getRow(hitr->first);

...
bool InttMap::RawDataWildcardComparator::operator() (
	...
	if(lhs.pid == Wildcard || rhs.pid == Wildcard)return false;
	if(lhs.pid != rhs.pid)return lhs.pid < rhs.pid;
	// if(lhs.pid != Wildcard && rhs.pid != Wildcard && lhs.pid != rhs.pid)return lhs.pid < rhs.pid;

	if(lhs.fee == Wildcard || rhs.fee == Wildcard)return false;
	if(lhs.fee != rhs.fee)return lhs.fee < rhs.fee;
...
bool InttMap::OfflineComparator::operator() (
	...
	if(lhs.strip_z != rhs.strip_z)return lhs.strip_z < rhs.strip_z;
	if(lhs.strip_phi != rhs.strip_phi)return lhs.strip_phi < rhs.strip_phi;

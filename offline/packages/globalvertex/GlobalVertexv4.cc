...
GlobalVertexv4::GlobalVertexv4(const unsigned int id)
  : _id(id)
{
}

GlobalVertexv4::~GlobalVertexv4()
{
  GlobalVertexv4::Reset();
}

void GlobalVertexv4::Reset()
{
  for (auto& _vtx : _vtxs)
  {
...
void GlobalVertexv4::identify(std::ostream& os) const
{
  os << "---GlobalVertexv4-----------------------" << std::endl;
...
int GlobalVertexv4::isValid() const
{
  if (_vtxs.empty())
  {
    return 0;
...
void GlobalVertexv4::insert_vtx(GlobalVertex::VTXTYPE type, const Vertex* vertex)
{
...
void GlobalVertexv4::clone_insert_vtx(GlobalVertex::VTXTYPE type, const Vertex* vertex)
{
  auto it = _vtxs.find(type);
...
size_t GlobalVertexv4::count_vtxs(GlobalVertex::VTXTYPE type) const
{
  auto it = _vtxs.find(type);
  if (it == _vtxs.end())
  {
...
float GlobalVertexv4::get_x() const { return get_position(0); }
float GlobalVertexv4::get_y() const { return get_position(1); }
float GlobalVertexv4::get_z() const { return get_position(2); }

float GlobalVertexv4::get_position(unsigned int coor) const
{
  ...
  if (svtxit == _vtxs.end())
  {
    ...
    if (mbdit == _vtxs.end())
    {
      ...
      GlobalVertex::ConstVertexIter caloit = find_vertexes(GlobalVertex::VTXTYPE::CALO);
      if (caloit != _vtxs.end())
	{
	  return caloit->second[0]->get_position(coor);
	}
      caloit = find_vertexes(GlobalVertex::VTXTYPE::CALO_JETSKEW);
      if (caloit != _vtxs.end())
	{
	  return caloit->second[0]->get_position(coor);
	}

      caloit = find_vertexes(GlobalVertex::VTXTYPE::CALO_AVGZ);
      if (caloit != _vtxs.end())
	{
	  return caloit->second[0]->get_position(coor);
	}

      caloit = find_vertexes(GlobalVertex::VTXTYPE::CALO_JETMLP);
      if (caloit != _vtxs.end())
	{
	  return caloit->second[0]->get_position(coor);
	}
...
float GlobalVertexv4::get_chisq() const
{
  auto svtxit = _vtxs.find(GlobalVertex::VTXTYPE::SVTX);
  if (svtxit == _vtxs.end())
  {
...
unsigned int GlobalVertexv4::get_ndof() const
{
  auto svtxit = _vtxs.find(GlobalVertex::VTXTYPE::SVTX);
  if (svtxit == _vtxs.end())
  {
...
float GlobalVertexv4::get_error(unsigned int i, unsigned int j) const
{
  auto svtxit = _vtxs.find(GlobalVertex::VTXTYPE::SVTX);
  if (svtxit == _vtxs.end())
  {

...
namespace
{
  ...
  //! pt
  template<class T> T get_pt( const T& px, const T& py ) { return std::sqrt( px*px + py*py ); }
  ...
  double fit_function_2d( double* x, double* par )
  {
    ...
    const double z    = x[1];
    if( ieta < 0 || ieta > 1 ) { TF2::RejectPoint(); return 0.; }
  ...
  for( int ieta = 0; ieta < 2; ++ieta )
    {
      ...
      leg->SetHeader( Form( "%s   entries: %i   v_{in}=%.4f cm/ns",
			    k_eta_labels[ieta], nEntries, m_drift_velocity ), "C" );
  ...
  for( const auto& [track_id, track] : *m_track_map )
      ...
      for( const auto* seed : { track->get_silicon_seed(), track->get_tpc_seed() } )
	{
	  ...
	  for( auto it = seed->begin_cluster_keys(); it != seed->end_cluster_keys(); ++it )
		...
		{
		case TrkrDefs::tpcId:  ++n_tpc;  break;

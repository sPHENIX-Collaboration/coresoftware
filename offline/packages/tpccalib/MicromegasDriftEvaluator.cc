...
namespace
{
  ...
  template<class T> inline T get_r(T x, T y) { return std::sqrt(square(x) + square(y)); }
    ...
    TVector3& intersect)
  {
    ...
    auto df = [&](double t)
    {
      ...
      double yt = Y0 + R * std::sin(t);
      double Rt = std::sqrt(xt*xt + yt*yt);
      return -ntile.X() * R * std::sin(t)
             + ntile.Y() * R * std::cos(t)
             + ntile.Z() * R * slope_rz * (Y0*std::cos(t) - X0*std::sin(t)) / Rt;
    };
  ...
  double fit_function_2d(double* x, double* par)
  {
    ...
    const double z     = x[1];
    if (itile < 0 || itile >= 8) { TF2::RejectPoint(); return 0.; }
  ...
  for (int j = 0; j < 8; ++j)
    {
      ...
      leg->SetHeader(Form("%i entries, v_{in}=%.2f m/ms", nEntries, m_drift_velocity * 1e5), "C");
      leg->AddEntry(h_fit_proj, "Gaussian slice mean", "p");
      leg->AddEntry(f1d,Form("slope=%.4f  v_{new}=%.3f#pm%.3f m/ms",slope, new_drift * 1e5, drift_err * 1e5), "l");
      leg->Draw();
    }
...
void MicromegasDriftEvaluator::evaluate_tracks()
{
  ...
  for (const auto& [track_id, track] : *m_track_map)
  {
      ...
      for (auto it = seed->begin_cluster_keys(); it != seed->end_cluster_keys(); ++it)
      {
        ...
        switch (detid)
        {
          case TrkrDefs::tpcId:
            ...
            {
              const auto cl  = m_cluster_map->findCluster(ckey);
              if (cl)
              {
            ...
            break;
          case TrkrDefs::mvtxId:       ++n_mvtx; break;
          case TrkrDefs::inttId:       ++n_intt; break;
          case TrkrDefs::micromegasId: ++n_mm;   break;
          default: break;
        }
      }
    }
	...
	TrackStruct ts;
	ts._chisquare            = track->get_chisq();
	ts._ndf                  = track->get_ndf();
	ts._nclusters_tpc        = n_tpc;
	ts._nclusters_mvtx       = n_mvtx;
	ts._nclusters_intt       = n_intt;
	ts._nclusters_micromegas = n_mm;

	ts._trk_state_z._layer   = mm_layer;
	ts._trk_state_z._tile    = tileid;
	ts._trk_state_z._z       = intersection.z();
	ts._trk_state_z._y_local = y_local;

	ts._found_cluster_z = best_cluster;

	const double z_track   = ts._trk_state_z._z;
	const double z_cluster = ts._found_cluster_z._z;
	m_hist3D->Fill(tileid + 0.5, z_track, z_track - z_cluster);

	m_container->add_track(ts);
	break;  
      }
  }
}

...
Connection *SpinDBOutput::ConnectDB()
{
  Connection *con=nullptr;
  try
  {
    con=DriverManager::getConnection(db_name,user_name,"");
  }
...
int SpinDBOutput::StoreDBContent(int run1,int run2,int qa_level)
{
  ...
  */
  ...
  while(true)
  {
    ...
    {
      spin_cont_store.insert(pair<int,SpinDBContent>(runnum,spin_cont));
    }
  }
...
int SpinDBOutput::GetArray(ResultSet *rs,const char *name, unsigned int *value,int nvalue)
{
  vector<string> svalue(nvalue,"");
  int ret=GetArray(rs,name,svalue);
  for(int i = 0; i < nvalue; i++)
...
int SpinDBOutput::GetArray(ResultSet *rs, const char *name, long long *value, int nvalue)
{
  vector<string> svalue(nvalue,"");
  int ret=GetArray(rs,name,svalue);
  for(int i = 0; i < nvalue; i++)

...
class SvtxVertexMap : public PHObject
{
 public:
  ...
  virtual const SvtxVertex* get(unsigned int) const { return nullptr; }
  virtual SvtxVertex* get(unsigned int) { return nullptr; }

  //! Add vertex to container. Note the container take to ownership
  virtual SvtxVertex* insert(SvtxVertex*) { return nullptr; }
  //! legacy interface. Add vertex to container. Note the container does not take ownership
  virtual SvtxVertex* insert_clone(const SvtxVertex*) { return nullptr; }

  virtual size_t erase(unsigned int) { return 0; }

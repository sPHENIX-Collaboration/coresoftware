...
MvtxThresholds::MvtxThresholds(const string &name) :
  SubsysReco(name),
  _fraction_of_mip(),
...
int MvtxThresholds::InitRun(PHCompositeNode* topNode) {
  ...
  if (verbosity > 0) {
    cout << "====================== MvtxThresholds::InitRun() ======================" << endl;
...
int MvtxThresholds::process_event(PHCompositeNode *topNode)
{
  _timer.get()->restart();
...
int MvtxThresholds::End(PHCompositeNode* topNode) {

  return Fun4AllReturnCodes::EVENT_OK;
}

void MvtxThresholds::CalculateMapsLadderThresholds(PHCompositeNode* topNode) {

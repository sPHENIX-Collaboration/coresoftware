...
void MvtxHitMap::add_hit(MvtxPixelDefs::pixelkey key, uint32_t nhits)
{
    ...
    is_sorted = false;
    auto it = std::find_if(m_pixel_hit_vector.begin(), m_pixel_hit_vector.end(), [key](const pixel_hits_pair_t& element) { return element.first == key; });
    ...
    else
    {
        ...
        m_pixel_hit_vector.push_back(std::make_pair(key, nhits));
    }
...
MvtxPixelDefs::pixelkey MvtxHitMap::get_most_significant_pixel() 
{
    ...
    if(m_pixel_hit_vector.empty()) { return MvtxPixelDefs::VOID_PIXEL; }
...
void MvtxHitMap::sort_by_hits()
{
    ...
    std::sort(m_pixel_hit_vector.begin(), m_pixel_hit_vector.end(), [](const pixel_hits_pair_t& a, const pixel_hits_pair_t& b) { return a.second > b.second; });
    is_sorted = true;
    return;
}

...
DetermineEventRho::DetermineEventRho(const std::string &name) : SubsysReco(name)
{
  // silence output from fastjet
  fastjet::ClusterSequence clusseq;
  if (Verbosity() > 0) { clusseq.print_banner(); }  
  else {
...
int DetermineEventRho::InitRun(PHCompositeNode *topNode)
{
  ...
  // set jet_eta_range
  if ( m_abs_jet_eta_range < 0 ) { m_abs_jet_eta_range = m_abs_input_eta_range - m_par; }
  if (Verbosity() > 0) { print_settings(); }
...
int DetermineEventRho::process_event(PHCompositeNode *topNode)
{
  ...
  if (Verbosity() > 1) { std::cout << "DetermineEventRho::process_event -- entered" << std::endl; }
  ...
  for ( unsigned int ipart = 0; ipart < particles.size(); ++ipart ) {
    ...
    if (this_e == 0.){ continue; } // skip zero energy particles
    ...
    // eta cut
    if (  std::abs(pseudojet.eta()) > m_abs_input_eta_range ) { continue; }
...
int DetermineEventRho::CreateNodes(PHCompositeNode *topNode)
{
  ...
  if ( !dstNode ){
    ...
    if ( !bkgNode ){ // create the node if it does not exist
...
float DetermineEventRho::CalcPercentile(const std::vector<float> & sorted_vec, const float percentile, const float nempty) const
{
  ...
  if ( sorted_vec.size() > 0 ) { 
    ...
    if ( perc_pos >= 0 && njets > 1) {
      ...
      if ( pindex + 1 > njets - 1 ) { // avoid out of range
        pindex = njets - 2;
        perc_pos = njets - 1;
      }
    ...
    else if ( perc_pos > -0.5 && njets >= 1 ) { result = sorted_vec.at(0); } 
    else { result = 0; } // end of if criteria

...
int IdealPadMap::load_from_cdb(const int /*verbosity*/)
{
  ...
  for (unsigned int ilayer = 0; ilayer < N_LAYERS; ++ilayer)
  {
    ...
    for (const double r : radius_mm_by_layer[ilayer])
    {
      radius_mm += r;
    }
                            ...
                            const unsigned int local_phibin) const
{
  ...
  if (side == 1U)
  {
    ...
    if (local_phibin >= pads_per_sector)
    {
      return std::numeric_limits<double>::quiet_NaN();
    }
  ...
  const double phi = ((side == 1U ? 1.0 : -1.0) * (cdb_phi - M_PI / 2.0)) + (static_cast<double>(mapped_sector) * M_PI / 6.0);

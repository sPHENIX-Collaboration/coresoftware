...
BbcGeomV1::BbcGeomV1()
{
  ...
  for (unsigned int ipmt=0; ipmt<128; ipmt++)
  {
    ...
    BbcGeomV1::set_xyz( ipmt, tube_x, tube_y, tube_z );
  }

}

void BbcGeomV1::set_xyz(const unsigned int ipmt, const float x, const float y, const float z)
{
    pmt_x[ipmt] = x;
    pmt_y[ipmt] = y;
    pmt_z[ipmt] = z;
    pmt_r[ipmt] = sqrt( x*x + y*y );
    pmt_phi[ipmt] = atan2(y,x);
}

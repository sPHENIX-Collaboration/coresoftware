...
void  BbcSig::SetTemplateMinMaxGoodADC(const Double_t min, const Double_t max)
{
  template_min_good_amplitude = min;
  template_max_good_amplitude = max;
}

void  BbcSig::SetTemplateMinMaxFitRange(const Double_t min, const Double_t max)
{
...
void BbcSig::MakeAndWriteTemplate(ostream& out, ostream& oerr)
{
  //verbose = 100;
  if ( verbose ) cout << "In  BbcSig::MakeAndWriteTemplate" << endl;

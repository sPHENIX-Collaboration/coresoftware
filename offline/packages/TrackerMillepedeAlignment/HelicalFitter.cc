	  ...
	  if(layer < 7)
	    {
	      if(layer > 2)
		{
		  Acts::Vector3 surf_center =  surf->center(_tGeometry->geometry().getGeoContext()) * 0.1;
		  float phi = atan2(surf_center(1), surf_center(0));
		  std::cout << "Surface location " << surf_center(0) << "  "  << surf_center(1) << "  " << surf_center(2) 
			    << " phi " <<   phi*180/3.14159 << std::endl;
		}

...
FitErrorCalculator::FitErrorCalculator (
	std::vector<ClusterFitPoint> const& points
) : m_points{points} {}

double
FitErrorCalculator::operator() (
	double const* params
) const {
	// xy-intercept
	Eigen::Vector3d b {
...
WeightedFitter::get_points (
	...
	for (
		...
		auto local_to_global_transform = surf->transform(m_geometry->geometry().getGeoContext());

		// From HelicalFitter::get_projectionXY
  		Acts::Vector3 const sensorCenter = surf->center(m_geometry->geometry().getGeoContext()) / Acts::UnitConstants::cm;  // convert to cm
		Acts::Vector3 const Z = -surf->normal(m_geometry->geometry().getGeoContext(), Acts::Vector3(1, 1, 1), Acts::Vector3(1, 1, 1));
		...
		Acts::Vector3 xglob = surf->transform(m_geometry->geometry().getGeoContext()) * (xloc * Acts::UnitConstants::cm);
		xglob /= Acts::UnitConstants::cm;
		Acts::Vector3 const yloc(0.0, 1.0, 0.0);
		Acts::Vector3 yglob = surf->transform(m_geometry->geometry().getGeoContext()) * (yloc * Acts::UnitConstants::cm);
		...
		Eigen::Vector3d local_pos = Eigen::Vector3d {
			cluster->getLocalX(),
			cluster->getLocalY(), // TPC/TPOT case handled below
			0.0
		...
		ClusterFitPoint point {
			...
			.o = sensorCenter, //  local_to_global_transform.translation(),
			.x = X, //  local_to_global_transform.rotation().col(0),
			.y = Y, //  local_to_global_transform.rotation().col(1),
			.z = Z, // -local_to_global_transform.rotation().col(2), // deliberate but idk why it's a left-handed coordinate system
		...
		switch (static_cast<TrkrDefs::TrkrId>(TrkrDefs::getTrkrId(*cluster_key_itr))) {
			...
			case TrkrDefs::tpcId: {
				...
				// need also distortion and global corrections a la HelicalFitter
				...
				unsigned int const side = TpcDefs::getSide(*cluster_key_itr);
				if (side == 0)
				{
				  zloc = -zloc;
				}
	...
	if (m_points.size() < 5) { return true; }
	// if (n_mvtx < 3) { return true; }
	// if (n_intt < 2) { return true; }
	// if (n_tpc < 30) { return true; }
...
WeightedFitter::draw (
	...
	double range = 15;
	graph->GetXaxis()->SetLimits(-range, range);
	graph->GetXaxis()->SetRangeUser(-range, range);
	graph->GetYaxis()->SetLimits(-range, range);
	graph->GetYaxis()->SetRangeUser(-range, range);
...
WeightedFitter::draw_cluster_xy (
	...
	std::array<Eigen::Vector3d, 2> endpoint = {
		cluster.pos - 1.0E+4 * cluster.sigma_x * cluster.x,
		cluster.pos + 1.0E+4 * cluster.sigma_x * cluster.x,
	};

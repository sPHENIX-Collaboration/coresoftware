  ...
  while ( fp = framelist[i++] )
      ...
      while (  pp !=  ptrFailure) 
	{
	  if (getPacketStructure(pp) == Unstructured)
	    {
	      if (getUnstructPacketHitFormat(pp) == oldhf) 
		setUnstructPacketHitFormat(pp,newhf);
	    }
...
void exitmsg()
{
  COUT << "** usage: changehitformat infile outfile old new [old new...] " << std::endl;
  exit(0);
}
...
main(int argc, char *argv[])
{
  ...
  while (  ( evt = it->getNextEvent())  )
	...
	nevt->change_hf(idold[j] , idnew[j]);

...
//____________________________________________________________________________..
MicromegasDetector::MicromegasDetector(PHG4Subsystem *subsys,
...
void MicromegasDetector::ConstructMe(G4LogicalVolume *logicWorld)
{
 ...
 // Do not forget to multiply the parameters with their respective CLHEP/G4 unit !
  ...
  G4VSolid *solidbox = new G4Box("MicromegasSolid", xdim / 2., ydim / 2., zdim / 2.);
  G4LogicalVolume *logical = new G4LogicalVolume(solidbox, G4Material::GetMaterial(m_Params->get_string_param("material")), "MicromegasLogical");
  ...
  G4VPhysicalVolume *phy = new G4PVPlacement(
      ...
      logical, "Micromegas", logicWorld, 0, false, OverlapCheck());

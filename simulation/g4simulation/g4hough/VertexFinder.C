...
///
bool VertexFinder::findVertex(vector<Track3D>& tracks, vector<float>& vertex, float sigma, bool fix_xy)
{
  vector<Matrix<float,5,5> > covariances;
  return findVertex(tracks, covariances, vertex, sigma, fix_xy);
}

bool VertexFinder::findVertex(vector<Track3D>& tracks, vector<Matrix<float,5,5> >& covariances, vector<float>& vertex, float sigma, bool fix_xy)
{

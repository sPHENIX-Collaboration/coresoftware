...
bool
PHG4SvtxClusterizer::ladder_lessthan(const PHG4Cell* lhs,
					  const PHG4Cell* rhs)
{

  if( PHG4CellDefs::SizeBinning::get_phibin(lhs->get_cellid()) < PHG4CellDefs::SizeBinning::get_phibin(rhs->get_cellid()) ) return true;
  else if( PHG4CellDefs::SizeBinning::get_phibin(lhs->get_cellid()) == PHG4CellDefs::SizeBinning::get_phibin(rhs->get_cellid()) )
    {
      if( PHG4CellDefs::SizeBinning::get_zbin(lhs->get_cellid()) < PHG4CellDefs::SizeBinning::get_zbin(rhs->get_cellid()) ) return true;
    }
  return false;
}

bool
PHG4SvtxClusterizer::maps_ladder_lessthan(const PHG4Cell* lhs,
					  const PHG4Cell* rhs)
{
...
bool
PHG4SvtxClusterizer::ladder_are_adjacent(const PHG4Cell* lhs, 
				       const PHG4Cell* rhs,
                                       const int &nphibins) {
  ...
  if (get_z_clustering(lhs_layer)) {
    if( fabs(PHG4CellDefs::SizeBinning::get_zbin(lhs->get_cellid()) - PHG4CellDefs::SizeBinning::get_zbin(rhs->get_cellid())) <= 1 ) 
      {
	if( fabs(PHG4CellDefs::SizeBinning::get_phibin(lhs->get_cellid()) - PHG4CellDefs::SizeBinning::get_phibin(rhs->get_cellid()) ) <= 1 )
	  {
	    return true;
	  } 
	else if(PHG4CellDefs::SizeBinning::get_phibin(lhs->get_cellid()) == 0 || PHG4CellDefs::SizeBinning::get_phibin(rhs->get_cellid()) == 0) 
	  {
	    if(fabs(PHG4CellDefs::SizeBinning::get_phibin(lhs->get_cellid()) - PHG4CellDefs::SizeBinning::get_phibin(rhs->get_cellid())) == (nphibins-1))
	      {
		return true;
	      }
	  }
    ...
    {
      if( fabs(PHG4CellDefs::SizeBinning::get_zbin(lhs->get_cellid()) - PHG4CellDefs::SizeBinning::get_zbin(rhs->get_cellid())) == 0 ) 
	{
	  if( fabs(PHG4CellDefs::SizeBinning::get_phibin(lhs->get_cellid()) - PHG4CellDefs::SizeBinning::get_phibin(rhs->get_cellid())) <= 1 )
	    {
	      return true;
...
bool PHG4SvtxClusterizer::ladder_are_adjacent(const PHG4Cell* lhs, 
					      const PHG4Cell* rhs) {
  int lhs_layer = lhs->get_layer();
  int rhs_layer = rhs->get_layer();
  if (lhs_layer != rhs_layer) return false;
...
void 
PHG4SvtxClusterizer::CalculateMapsLadderThresholds(PHCompositeNode *topNode) {
  ...
  for(PHG4CylinderCellGeomContainer::ConstIterator layeriter = layerrange.first;
    ...
    for(unsigned int i=0; i<cell_list.size(); i++) {
      for(unsigned int j=i+1; j<cell_list.size(); j++) {
        if( ladder_are_adjacent(cell_list[i], cell_list[j], nphibins) )
          add_edge(i,j,G);
      }
...
void
PHG4SvtxClusterizer::ClusterMapsLadderCells(PHCompositeNode *topNode) {

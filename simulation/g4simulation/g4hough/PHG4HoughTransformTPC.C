...
int PHG4HoughTransformTPC::process_event(PHCompositeNode *topNode)
{
    ...
    cout<<"pass 1 : "<<_tracks.size()<<" tracks"<<endl;

    vector<SimpleTrack3D> store_tracks = _tracks;
    vector<HelixKalmanState> store_states = _tracker->getKalmanStates();
    _tracker->clear();
    _tracks.clear();
    _tracker->findHelices(_clusters_init, _min_hits_init, _max_hits_init, _tracks);
    cout<<"pass 2 : "<<_tracks.size()<<" tracks"<<endl;
    for(unsigned int tr=0;tr<store_tracks.size();++tr)
    {
      _tracks.push_back( store_tracks[tr] );
      _tracker->getKalmanStates().push_back( store_states[tr] );
    }

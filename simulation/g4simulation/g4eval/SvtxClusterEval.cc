  ...
  */
  ...
  if(g4hitkeys.size()==1 ){
	...
	if(this_g4hit!=nullptr){
	  ...
	  TVector3 vec(this_g4hit->get_avg_x(), this_g4hit->get_avg_y(), this_g4hit->get_avg_z());
	  if(_verbosity > 2) {
	    cout << "layer: " << layer << " (" << glayer << ") " << " gtrackID: " << this_g4hit->get_trkid() << " novlp: " << ng4hit << " phi: " << vec.Phi() << " z: " << this_g4hit->get_avg_z() << " r: " << vec.Perp() << " keyg4: " << g4hitkey << " cz: " << cluster->getZ() << endl; //<< " keyrec: "<< *it.second << endl;
}
	}
      }
      if(ng4hit>n_max){

...
SvtxEvaluator::SvtxEvaluator(const string &name, const string &filename) :
  ...
  _timer(PHTimeServer::get()->insert_new("SvtxEvaluator")),
  _nlayers(6),
  ...
  _filename = filename;
  _internal_timer.push_back(PHTimeServer::get()->insert_new("SvtxEvaluator get nodes"));
  _internal_timer.push_back(PHTimeServer::get()->insert_new("SvtxEvaluator printInputInfo()"));
  _internal_timer.push_back(PHTimeServer::get()->insert_new("SvtxEvaluator fillGtrackObjects()"));
  _internal_timer.push_back(PHTimeServer::get()->insert_new("SvtxEvaluator fillClusterToG4HitMap()"));
  _internal_timer.push_back(PHTimeServer::get()->insert_new("SvtxEvaluator fillTrackToGtrackMap()"));
  _internal_timer.push_back(PHTimeServer::get()->insert_new("SvtxEvaluator fillTrackFindingFields()"));
  _internal_timer.push_back(PHTimeServer::get()->insert_new("SvtxEvaluator fitGtrackProducedClusters()"));
  _internal_timer.push_back(PHTimeServer::get()->insert_new("SvtxEvaluator fillOutputNtuples()"));
  _internal_timer.push_back(PHTimeServer::get()->insert_new("SvtxEvaluator printOutputInfo()"));
}
...
int SvtxEvaluator::process_event(PHCompositeNode *topNode)
{
  ...
  if ((!_cellGeos)&&(!_ladderGeos)) {
    ...
    if (verbosity > 0 && firstevent) {
      cout << "SvtxEvaluator layers: " << _nlayers
...
int SvtxEvaluator::fillLayerTypeMap(PHCompositeNode *topNode) {
...
int SvtxEvaluator::fillGhitList(PHCompositeNode *topNode) {
...
void SvtxEvaluator::fillGtrackObjects()
{
  if(verbosity > 1) cout << "SvtxEvaluator::fillGtrackObjects() entered" << endl;
...
void SvtxEvaluator::fillCellToG4HitMap()
{
  if(verbosity > 1) cout << "SvtxEvaluator::fillCellToG4hitMap() entered" << endl;
...
void SvtxEvaluator::fillClusterToG4HitMap()
{
  // reset the map for this event
  _cluster_g4hit_mmap.clear();
  _g4hit_cluster_mmap.clear();
...
void SvtxEvaluator::fillTrackToGtrackMap()
{
  if(verbosity > 1) cout << "SvtxEvaluator::fillTrackToGtrackMap() entered" << endl;
...
void SvtxEvaluator::fillTrackPurityMap()
{
  _track_purity_map.clear();

  if(verbosity > 1) cout << "SvtxEvaluator::fillTrackPurityMap() entered" << endl;
  ...
  if (true) {
    ...
    for (std::set<PHG4Hit*>::iterator iter = g4hits.begin();
	 ...
	 ++iter) {
      ...
      if (i==0) cout << "g4hit eval limited to 100 entries" << endl;
      if (i > 100) break;
      ++i;
...
void SvtxEvaluator::fitGtrackProducedClusters() {

...
public:
  ...
  std::set<PHG4Particles*>  all_truth_particles (SvtxVertex* vertex);
...
private:
  ...
  std::map<PHG4VtxPoint*,std::set<SvtxVertex*> >               _cache_all_vertexes_from_particle;
  std::map<PHG4VtxPoint*,SvtxVertex* >                         _cache_best_vertex_from_particle;
  std::map<std::pair<SvtxVertex*,PHG4VtxPoint*>, unsigned int> _cache_get_nclusters_contribution;
};

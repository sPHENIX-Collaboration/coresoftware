...
int BaseTruthEval::get_embed(PHG4Particle* particle) {
  ...
  if (!has_reduced_node_pointers()) {++m_Errors; return 0;}
  
  if (m_Strict) {assert(particle);}
  else if (!particle) {++m_Errors; return 0;}
...
//  if (!is_primary(particle)) return 0;
  ...
  if (m_Strict) {assert(primary);}
  else if (!primary) {++m_Errors; return 0;}
...
PHG4VtxPoint* BaseTruthEval::get_vertex(PHG4Particle* particle) {
  ...
  if (!has_reduced_node_pointers()) {++m_Errors; return nullptr;} 
  
  if (m_Strict) {assert(particle);}
  else if (!particle) {++m_Errors; return nullptr;}

  PHG4VtxPoint* vtx = m_TruthInfo->GetVtx( particle->get_vtx_id() );
  if (m_Strict) {assert(vtx);}
  else if (!vtx) {m_Errors++;}
...
bool BaseTruthEval::is_primary(PHG4Shower* shower) {
  ...
  if (!has_reduced_node_pointers()) {++m_Errors; return false;}
  
  if (m_Strict) {assert(shower);}
  else if (!shower) {++m_Errors; return false;}
...
bool BaseTruthEval::is_primary(PHG4Particle* particle) {
  ...
  if (m_Strict) {assert(particle);}
  else if (!particle) {++m_Errors; return false;}
...
PHG4Shower* BaseTruthEval::get_primary_shower(PHG4Shower* shower) {
  ...
  if (m_Strict) {assert(shower);}
  else if (!shower) {++m_Errors; return nullptr;}
...
PHG4Particle* BaseTruthEval::get_primary_particle(PHG4Particle* particle) {
  ...
  if (m_Strict) {assert(returnval);}
  else if (!returnval) {++m_Errors;}
...
PHG4Particle* BaseTruthEval::get_primary_particle(PHG4Shower* shower) {
  ...
  if (m_Strict) {assert(parent_particle);}
  else if (!parent_particle) {++m_Errors; return nullptr;}
  
  PHG4Particle* primary_particle = get_primary_particle(parent_particle);

  if (m_Strict) {assert(primary_particle);}
  else if (!primary_particle) {++m_Errors; return nullptr;}
...
std::set<PHG4Shower*> BaseTruthEval::all_secondary_showers(PHG4Shower* shower) {
  ...
  if (!has_reduced_node_pointers()) {++m_Errors; return std::set<PHG4Shower*>();}
  
  if (m_Strict) {assert(shower);}
  else if (!shower) {++m_Errors; return std::set<PHG4Shower*>();}
...
PHG4Particle* BaseTruthEval::get_particle(PHG4Hit* g4hit) {
  ...
  if (m_Strict) {assert(g4hit);}
  else if (!g4hit) {++m_Errors; return nullptr;}
  
  PHG4Particle* particle = m_TruthInfo->GetParticle( g4hit->get_trkid() );
  if (m_Strict) {assert(particle);}
  else if (!particle) {++m_Errors;}
...
bool BaseTruthEval::has_reduced_node_pointers() {
  ...
  if (m_Strict) assert(m_TruthInfo);
  else if (!m_TruthInfo) return false;

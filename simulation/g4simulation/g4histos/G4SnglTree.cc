...
G4SnglTree::~G4SnglTree()
{
	...
	int
G4SnglTree::Init( PHCompositeNode* )
{
	...
	int
G4SnglTree::process_event( PHCompositeNode* topNode )
{
	...
	int
G4SnglTree::End(PHCompositeNode * topNode)
{
	outfile->cd();
	g4tree->Write();
	outfile->Write();
	...
	void
G4SnglTree::AddNode(const std::string &name, const int detid)
{
 ...
 int 
G4SnglTree::process_hit(PHG4HitContainer *hits, string dName, int detid, int &nhits)
{
	...
	if (hits)
	{
		...
		for (edepiter= layer_edep_map.begin(); edepiter != layer_edep_map.end(); edepiter++)
		{
			...
			if(!strcmp("G4HIT_CEMC",dName.c_str()))mG4EvtTree.cemcactESum[nLayers] = edepiter->second;
			else if(!strcmp("G4HIT_ABSORBER_CEMC",dName.c_str()))mG4EvtTree.cemcabsESum[nLayers] = edepiter->second;
			else if(!strcmp("G4HIT_HCAL",dName.c_str()))mG4EvtTree.hcalactESum[nLayers] = edepiter->second;
			else if(!strcmp("G4HIT_ABSORBER_HCAL",dName.c_str())) mG4EvtTree.hcalabsESum[nLayers] = edepiter->second;
		}
	}

...
int PHG4TpcPadPlaneReadout::CreateReadoutGeometry(PHCompositeNode *topNode, PHG4CylinderCellGeomContainer *seggeo)
{
  ...
  {
    for (int layer = MinLayer[imodule]; layer < MinLayer[imodule] + NTpcLayers[imodule]; ++layer)
    {

      double layer_radius=MinRadius[imodule] + ((double) (layer - MinLayer[imodule]) + 0.5) * Thickness[imodule]
      if (Verbosity())
      {
        cout << " layer " << layer << " MinLayer " << MinLayer[imodule] << " region " << imodule
  	     << " radius " <<layer_radius
             << " thickness " << Thickness[imodule]
             << " NZbins " << NZBins << " zmin " << MinZ << " zstep " << ZBinWidth
             << " phibins " << NPhiBins[imodule] << " phistep " << PhiBinWidth[imodule] << endl;
      }



      PHG4CylinderCellGeom_Sectored *layerseggeo=new PHG4CylinderCellGeom_Sectored();
      //PHG4CylinderCellGeom *layerseggeo = new PHG4CylinderCellGeom();
      
      layerseggeo->set_layer(layer);
      layerseggeo->set_radius(layer_radius);
      layerseggeo->set_thickness(Thickness[imodule]);
      ...
      layerseggeo->set_phibins_per_sector(NPhiBins[imodule]/12);
      layerseggeo->set_phistep(PhiBinWidth[imodule]);
      layerseggeo->set_sectors(12);
      layerseggeo->set_sectormargin(asin(TotalFrameWidth/layer_radius);//dead area in phi isrelated to radius and dead area in rphi.
      ...
      if (NPhiBins[imodule] * NZBins > 5100000)
      {
        cout << "increase Tpc cellsize, number of cells "
             << NPhiBins[imodule] * NZBins << " for layer " << layer
...
{
  // The coordinates x_gem and y_gem are the cartesian position of the electron when it arrives at the gem stack
  // The coordinate z_gem reflects the apparent z position that the electron came from, accounting for drift diffusion.
  ...
  //layers aren't entirely hermetic, so we may miss some electrons:
  if (layernum == 0)
  {
    return;
  }
  ...
  const int phibins = LayerGeom->get_phibins();
  const int zbins = LayerGeom->get_zbins();
...
{
  //distribute the share of the overall charge into phibins around the true phi value using the cloud width
  //assuming each pad is an r-phi rectangle
...
void PHG4TpcPadPlaneReadout::populate_zigzag_phibins(const unsigned int layernum, const double phi, const double cloud_sig_rp, std::vector<int> &pad_phibin, std::vector<double> &pad_phibin_share)
{
  ...
  // Assuming a gaussian charge distribution in the avalanche,
  ...
  // rcc note that this is inaccurate if we are near a frame on either side.
  ...
  // Find the pad range that covers this phi range (note that it may wrap around, or crash here if get_phibin doesn't allow phi out of bounds)
  // rcc note that if the bin is over a frame, we won't get as many bins as we were expecting.
  ...
  // rcc note that the pad extent is not necessarily constant.
  const double pad_rphi = 2.0 * LayerGeom->get_phistep() * radius;
  // rcc: the assumption on the previous line only works for pads that do not span a gap!


  //generate the centers and widths of each pad:
  using PadParameterSet = std::array<double,2>;//ad hoc container for {pad half-width in rphi, pad center in rphi}
...
void PHG4TpcPadPlaneReadout::SetDefaultParameters()
{
  ...
  //the next five are taken from Feb 12 drawings of the frames
  set_default_double_param("tpc_minradius_mid", 41.14);
  set_default_double_param("tpc_minradius_outer", 58.35);

  set_default_double_param("tpc_maxradius_inner", 40.26);  // cm
  set_default_double_param("tpc_maxradius_mid", 57.49);
  set_default_double_param("tpc_maxradius_outer", 75.84);  // from Tom
  ...
  set_default_int_param("ntpc_sectors",12);
  set_default_double_param("tpc_frame_side_gap",0.08); //cm space between radial line and start of frame
  set_default_double_param("tpc_frame_side_width",0.26); //cm thickness of frame
...
void PHG4TpcPadPlaneReadout::UpdateInternalParameters()
{
  ...
  TotalFrameWidth=get_double_param("tpc_frame_side_gap")+get_double_param("tpc_frame_side_width"); 

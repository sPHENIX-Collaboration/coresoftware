...
void PHG4TpcPadPlaneReadout::SetDefaultParameters()
{
  ...
  set_default_double_param("tpc_sector_phi_bias_inner", 0.0);//-0.0041);//sector bias in phi for R1 sector
  set_default_double_param("tpc_sector_phi_bias_mid",   0.0);//-0.0029);//sector bias in phi for R2 sector
  set_default_double_param("tpc_sector_phi_bias_outer", 0.0);//-0.0);   //sector bias in phi for R3 sector

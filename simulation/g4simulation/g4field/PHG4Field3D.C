...
{    
  ...
  // phi.
  if( verb_>0 ) { G4cout << "  --> Sorting Entries..." << endl; }
  ...
  if(verb_>0){ G4cout << "  --> Putting entries into containers... " <<  endl; }
...
void PHG4Field3D::GetFieldValue(const double point[4], double *Bfield ) const
{
  if(verb_>2)
    G4cout << "\nPHG4Field3D::GetFieldValue" << endl;
  double x = point[0];
  double y = point[1];
  double z = point[2];
  ...
  if(verb_>2){ 
    G4cout << "END PHG4Field3D::GetFieldValue\n" 
           << "  --->  {Bx, By, Bz} : "
...
void PHG4Field3D::GetFieldCyl( const double CylPoint[4], double *BfieldCyl ) const
{
  float z = CylPoint[0]; 
  float r = CylPoint[1];
...
bool PHG4Field3D::bin_search( const vector<float>& vec, unsigned start, unsigned end, const float& key, unsigned& index ) const { 
...
void PHG4Field3D::print_map( map<trio,trio>::iterator& it ) const {
...
#ifndef  __CINT__
PHG4Field3DWrapper::PHG4Field3DWrapper(const string &filename) :
  field(filename)
{}

double PHG4Field3DWrapper::GetBx(double x, double y, double z)
{
...
double PHG4Field3DWrapper::GetBy(double x, double y, double z)
{
  double pos[4] = {x*cm, y*cm, z*cm, 0};
  double Bfield[6] = {0};
  field.GetFieldValue(pos,Bfield);
...
double PHG4Field3DWrapper::GetBz(double x, double y, double z)
{
  double pos[4] = {x*cm, y*cm, z*cm, 0};
  double Bfield[6] = {0};
  field.GetFieldValue(pos,Bfield);
...
double PHG4Field3DWrapper::GetBCylZ(double z, double r, double phi) 
{
...
double PHG4Field3DWrapper::GetBCylR(double z, double r, double phi) 
{
  double pos[4] = {z*cm, r*cm, phi*rad, 0};
  double Bfield[6] = {0};
  field.GetFieldCyl(pos,Bfield);
...
double PHG4Field3DWrapper::GetBCylPHI(double z, double r, double phi) 
{
  double pos[4] = {z*cm, r*cm, phi*rad, 0};
  double Bfield[6] = {0};
  field.GetFieldCyl(pos,Bfield);

/*  2023.02.26 by David Stewart
 *  See comments in associated header file.
 */

#include "InttClusterizer_TruthTrack.h"
...
#include <trackbase/TrkrTruthTrackv1.h> // will generate a new TrkrTruthTracks
                                        // module will require but will NOT generate a 
                                        // new TrkrTruthTrackContainer. This module must be run 
                                        // after the PHG4TpcElectronDrift module
...
bool InttClusterizer_TruthTrack::ladder_are_adjacent( const std::pair<TrkrDefs::hitkey, TrkrHit*> &lhs, const std::pair<TrkrDefs::hitkey, TrkrHit*> &rhs, const int layer)
{
...
bool InttClusterizer_TruthTrack::ladder_are_adjacent( RawHit* lhs, RawHit* rhs, const int layer)
{
  if (get_z_clustering(layer))
    {
...
InttClusterizer_TruthTrack::InttClusterizer_TruthTrack(const string& name,
  ...
  , m_clusterlist(nullptr)
    // cluster hit association not stored for truth tracks
...
int InttClusterizer_TruthTrack::InitRun(PHCompositeNode* topNode)
{
  /*
  ...
  */
  ...
  // Get the TrkrTruthClusterContainer; if it does not exist, abort the run.
  // This required that the PHG4TpcElectronDrift node has already been run.

  // The cluster and association 
  auto trkrclusters = findNode::getClass<TrkrClusterContainer>(dstNode, "TRKR_TRUTHCLUSTERCONTAINER");
  if (!trkrclusters)
  {
    cout << PHWHERE << "TRKR_TRUTHCLUSTERCONTAINER Node missing; make sure PHG4TpcElectronDrift module has already been run. Doing nothing." << endl;
    return Fun4AllReturnCodes::ABORTRUN;
  }
  ...
  if (Verbosity() > 0)
  {
    cout << "====================== InttClusterizer_TruthTrack::InitRun() =====================" << endl;
...
int InttClusterizer_TruthTrack::process_event(PHCompositeNode* topNode)
{
  cout << " FIXME starting InttClusterizer_TruthTrack " << endl;
  ...
  m_clusterlist = findNode::getClass<TrkrClusterContainer>(topNode, "TRKR_TRUTHCLUSTERCONTAINER");
  if (!m_clusterlist)
  {
    cout << PHWHERE << " ERROR: Can't find TRKR_TRUTHCLUSTERCONTAINER." << endl;
    return Fun4AllReturnCodes::ABORTRUN;
  }

  // get the truth track container
  m_truthtracks = findNode::getClass<TrkrTruthTrackContainer>(topNode, "TRKR_TRUTHTRACKCONTAINER");
  if (!m_truthtracks)
  {
    cout << PHWHERE << " ERROR: Can't find TRKR_TRUTHTRACKCONTAINER." << endl;
    return Fun4AllReturnCodes::ABORTRUN;
  }

  // information for which hits come from truth tracks
  ...
  // get node for cluster hit associations // removed for truth tracks
...
void InttClusterizer_TruthTrack::CalculateLadderThresholds(PHCompositeNode* topNode)
{
  /*
...
void InttClusterizer_TruthTrack::ClusterLadderCells(PHCompositeNode* topNode)
{
  if (Verbosity() > 0)
    cout << "Entering InttClusterizer_TruthTrack::ClusterLadderCells " << endl;
  ...
  for (TrkrHitSetContainer::ConstIterator hitsetitr = hitsetrange.first;
       ...
       ++hitsetitr)
  {
    cout << hitsetitr->first.get_trkid() << endl;
    // Each hitset contains only hits that are clusterizable - i.e. belong to a single sensor
    TrkrHitSet *hitset = hitsetitr->second;

    if(Verbosity() > 1) cout << "InttClusterizer_TruthTrack found hitsetkey " << hitsetitr->first << endl;
    if (Verbosity() > 2)
      hitset->identify();
  ...
  /*     cout << "After InttClusterizer_TruthTrack, cluster-hit associations are:" << endl; */
...
void InttClusterizer_TruthTrack::ClusterLadderCellsRaw(PHCompositeNode* topNode)
{
  if (Verbosity() > 0)
    cout << "Entering InttClusterizer_TruthTrack::ClusterLadderCells " << endl;
...
void InttClusterizer_TruthTrack::PrintClusters(PHCompositeNode* topNode)
{
  if (Verbosity() > 1)
  {
    ...
    cout << "================= After InttClusterizer_TruthTrack::process_event() ====================" << endl;

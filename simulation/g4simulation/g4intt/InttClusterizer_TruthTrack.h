#ifndef INTT_INTTCLUSTERIZER_TRUTHTRACK_H
#define INTT_INTTCLUSTERIZER_TRUTHTRACK_H

/*  2023.02.26 by David Stewart
 *  This file is a copy of InttClusterizer.h, with the following minimal modifications.
 *  For the sake of not introducing bugs on the short time scale of coding, the code is
 *  deliberatly changed as little as possible. If the InttClusterizer.{h,cc} is modified 
 *  in the future for either efficiency or correctness, this current code whould be updated
 *  in parallel accordingly.
 *   - Consider only TrkrHit's from truth tracks
 *   - Put resulting clusters in the TRKR_TRUTHCLUSTERCONTAINER node
 *   - Update the TrkrTruthTrack's in the TRKR_TRUTHTRACKCONTAINER
 */
...
class InttClusterizer_TruthTrack : public SubsysReco
{
 public:
  InttClusterizer_TruthTrack(const std::string &name = "InttClusterizer_TruthTrack",
                  unsigned int min_layer = 0, unsigned int max_layer = UINT_MAX);
  ~InttClusterizer_TruthTrack() override {}
 ...
 private:
  ...
  TrkrClusterContainer *m_clusterlist; // for truth clusters
  // TrkrClusterHitAssoc *m_clusterhitassoc; // not stored for truth tracks

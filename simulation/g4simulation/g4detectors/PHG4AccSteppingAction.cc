...
PHG4AccSteppingAction::PHG4AccSteppingAction( PHG4AccDetector* detector ):
...
//____________________________________________________________________________..
bool PHG4AccSteppingAction::UserSteppingAction( const G4Step* aStep, bool )
{
  ...
  if ( isactive > PHG4AccDetector::INACTIVE ) 
    {
      bool geantino = false;
          ...
          aTrack->GetParticleDefinition()->GetParticleName().find("geantino") != string::npos)
	{
	  ...
	  if (hit->get_z(0) > zmax || hit->get_z(0) < zmin)
	    {
	      cout << "PHG4AccSteppingAction: hit outside acceptance, layer: " << layer_id << endl;
	      hit->identify();
	    }
          break;
        default:
	...
	{
	  cout << "PHG4AccSteppingAction: hit outside acceptance zmin " << zmin << ", zmax " << zmax << " at exit" << endl;
	  hit->identify();
	}
      if (geantino)
	{
...
//____________________________________________________________________________..
void PHG4AccSteppingAction::SetInterfacePointers( PHCompositeNode* topNode )
{
  ...
  if ( ! hits_ )
    {
      std::cout << "PHG4AccSteppingAction::SetTopNode - unable to find " << hitnodename << std::endl;
    }
  if ( ! absorberhits_)
    {
      if (verbosity > 0)
	{
	  std::cout << "PHG4AccSteppingAction::SetTopNode - unable to find " << absorbernodename << std::endl;
	}
    }
}

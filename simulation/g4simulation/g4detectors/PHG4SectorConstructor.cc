...
// geometry checks
  ...
  if ((geom.get_min_polar_edge() == Sector_Geometry::kFlatEdge
      or geom.get_max_polar_edge() == Sector_Geometry::kFlatEdge)
      and geom.get_N_Sector() <= 2)
    {
      G4Exception(
          (string("PHG4SectorConstructor::Construct_Sectors::") + (name_base)).c_str(),
          __FILE__, FatalException,
  ...
  const G4Transform3D transform_Det_to_Hall =
      G4RotateX3D(-geom.get_normal_polar_angle())
          * G4TranslateZ3D(
  ...
  const double sph_min_polar_angle =
      (geom.get_min_polar_edge() == Sector_Geometry::kConeEdge) ?
          geom.get_min_polar_angle() : 0;
  const double sph_max_polar_angle =
      (geom.get_max_polar_edge() == Sector_Geometry::kConeEdge) ?
          geom.get_max_polar_angle() : pi;
  ...
  if (geom.get_min_polar_edge() != Sector_Geometry::kConeEdge
      or geom.get_max_polar_edge() != Sector_Geometry::kConeEdge)
    {
      // build a flat edge
      ...
      const double sph_min_polar_size =
          (geom.get_min_polar_edge() == Sector_Geometry::kConeEdge) ?
              geom.get_max_R() :
              geom.get_normal_start()
                  * tan(
                      geom.get_normal_polar_angle()
                          - geom.get_min_polar_angle());
      const double sph_max_polar_size =
          (geom.get_max_polar_edge() == Sector_Geometry::kConeEdge) ?
              geom.get_max_R() :
              geom.get_normal_start()
                  * tan(
                      geom.get_max_polar_angle()
                          - geom.get_normal_polar_angle());
  ...
  for (Sector_Geometry::t_layer_list::const_iterator it =
      geom.layer_list.begin(); it != geom.layer_list.end(); ++it)
    {
      ...
      if (l.percentage_filled > 100. or l.percentage_filled < 0)
        {
          ...
          G4Exception(
              (string("PHG4SectorConstructor::Construct_Sectors::")
                  + (name_base)).c_str(), __FILE__, FatalException,
              s.str().c_str());
        }
  ...
  if (N_Sector <= 2)
    {
      ...
      return (get_normal_start() + get_total_thickness())
          * sqrt(1 + max_tan_angle * max_tan_angle);
  ...
  else
    {
      ...
      return (get_normal_start() + get_total_thickness())
          * sqrt(1 + pow(tan(max_angle), 2) + pow(tan(2 * pi / N_Sector), 2))
          * 2;
    }
...
void
Sector_Geometry::AddLayers_DriftVol_COMPASS(const double drift_vol_thickness)
{
...
void
Sector_Geometry::AddLayers_HBD_GEM(const int n_GEM_layers)
{
...
//! arXiv:1103.4277, doi:10.1016/j.nima.2011.04.015. 3.5.1
void
Sector_Geometry::AddLayers_HBD_Readout()
{
...
void
Sector_Geometry::AddLayers_AeroGel_ePHENIX(const double radiator_length,

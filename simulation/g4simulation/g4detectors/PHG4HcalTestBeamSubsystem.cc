#include "PHG4HcalTestBeamSubsystem.h"
...
PHG4HcalTestBeamSubsystem::PHG4HcalTestBeamSubsystem( const std::string &name, const int lyr ):
  PHG4Subsystem( name ),
  detector_( 0 ),
...
//_______________________________________________________________________
int PHG4HcalTestBeamSubsystem::Init( PHCompositeNode* topNode )
{
  PHNodeIterator iter( topNode );
  PHCompositeNode *dstNode = dynamic_cast<PHCompositeNode*>(iter.findFirst("PHCompositeNode", "DST" ));

  // create detector
  detector_ = new PHG4HcalTestBeamDetector(topNode, Name(), layer);
  ...
  if (active)
      ...
      if ( !block_hits )
	{
      ...
      steppingAction_ = new PHG4HcalTestBeamSteppingAction(detector_);
...
//_______________________________________________________________________
int
PHG4HcalTestBeamSubsystem::process_event( PHCompositeNode * topNode )
{
...
//_______________________________________________________________________
PHG4Detector* PHG4HcalTestBeamSubsystem::GetDetector( void ) const
{
    return detector_;
}

//_______________________________________________________________________
PHG4SteppingAction* PHG4HcalTestBeamSubsystem::GetSteppingAction( void ) const
{

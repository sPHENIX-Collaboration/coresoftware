...
/*
    void set_nominal_layer_radius(const G4double radius)
    {
      layer_nominal_radius = radius * mm;
    }
*/
    void set_radius_stagger(const G4double stagger)
    {
      radius_stagger = stagger * mm;
    }
    void set_N_staggers(const int nstag)
    {
      N_staggers = nstag;
    }
    void set_strip_tilt(const G4double tilt)
    {
      strip_tilt = tilt * rad;
    }
    void set_option_double_layer(const bool option)
    {
      option_double_layer = option;
    }
    void set_N_strips_in_sensor_phi(const int nstrips)
    {
      N_strips_per_column = nstrips;
    }
    void set_strip_pitch(const G4double pitch)
    {
      strip_y = pitch * mm;
    }
    void set_add_lower_roc(const bool option)
    {
      add_lower_roc = option;
    }
  ...
  private:
    ...
    //std::vector< std::pair<int,int> > vpair layerconfig_;
    vpair layerconfig_;

...
namespace PHG4Sector
{
  ...
  public:
    Layer( //! name base for this layer
        ...
        bool _active) :
        name(_name), material(_material), depth(_depth), percentage_filled(
            _percentage_filled), active(_active)
    {
    }
...
//   ! max radius from IP
    ...
    {
      set_normal_start(
          z_intercept / cos(angle_intercept)
              * cos(normal_polar_angle - angle_intercept));
    }
  ...
  public:
    ...
    AddLayers_AeroGel_ePHENIX(const double radiator_length = 2 * cm,
        const double expansion_length = 18 * cm, std::string radiator =
            "Default");

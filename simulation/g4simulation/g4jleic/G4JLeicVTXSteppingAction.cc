...
G4JLeicVTXSteppingAction::G4JLeicVTXSteppingAction(G4JLeicVTXDetector* detector, const PHParametersContainer* parameters)
...
G4JLeicVTXSteppingAction::~G4JLeicVTXSteppingAction()
{
...
//____________________________________________________________________________..
bool G4JLeicVTXSteppingAction::UserSteppingAction(const G4Step* aStep, bool was_used)
{
  G4TouchableHandle touch = aStep->GetPreStepPoint()->GetTouchableHandle();
  G4TouchableHandle touchpost = aStep->GetPostStepPoint()->GetTouchableHandle();
  // get volume of the current step
  G4VPhysicalVolume* volume = touch->GetVolume();
// IsInVTX(volume) returns
//  == 0 outside of pstof
//   > 0 for hits in active volume
//  < 0 for hits in passive material
  int whichactive = m_Detector->IsInVTX(volume);
  if (!whichactive)
  {
    return false;
  }
...
//____________________________________________________________________________..
void G4JLeicVTXSteppingAction::SetInterfacePointers(PHCompositeNode* topNode)
{
  string hitnodename;
  ...
  if (!m_HitContainer)
  {
    std::cout << "G4JLeicVTXSteppingAction::SetTopNode - unable to find " << hitnodename << std::endl;
  }
}

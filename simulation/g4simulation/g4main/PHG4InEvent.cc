...
int PHG4InEvent::AddVtx(const int /*id*/, const PHG4VtxPoint &vtx)
{
...
int PHG4InEvent::AddVtxCommon(PHG4VtxPoint *newvtx)
{
  ...
  for (map<int, PHG4VtxPoint *>::const_iterator viter = vtxbegin_end.first; viter != vtxbegin_end.second; ++viter)
  {
    ...
    bool xcomp = std::abs(newvtx->get_x() - viter->second->get_x()) <= distepsilon * (*std::max_element(xlist.begin(), xlist.end()));
    bool ycomp = std::abs(newvtx->get_y() - viter->second->get_y()) <= distepsilon * (*std::max_element(ylist.begin(), ylist.end()));

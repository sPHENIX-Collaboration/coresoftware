...
void PHG4PhenixEventAction::BeginOfEventAction(const G4Event* event)
{
  ...
  if (VERBOSE)
  {
    std::cout << "PHG4PhenixEventAction::BeginOfEventAction" << std::endl;
  }
  ...
  for (ActionList::const_iterator iter = actions_.begin(); iter != actions_.end(); ++iter)
  {
    if (*iter)
    {
      if (VERBOSE)
      {
        std::cout << "PHG4PhenixEventAction::BeginOfEventAction - processing " << *iter << std::endl;
      }
...
void PHG4PhenixEventAction::EndOfEventAction(const G4Event* event)
{
  if (VERBOSE)
  {
    std::cout << "PHG4PhenixEventAction::EndOfEventAction" << std::endl;
  }
  ...
  for (ActionList::const_iterator iter = actions_.begin(); iter != actions_.end(); ++iter)
  {
    if (*iter)
    {
      if (VERBOSE)
      {
        std::cout << "PHG4PhenixEventAction::EndOfEventAction - processing " << *iter << std::endl;
      }

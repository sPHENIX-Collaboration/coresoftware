...
ClassImp(PHG4Showerv2);

PHG4Showerv2::PHG4Showerv2()
  : _id(0xFFFFFFFF),
    _pos(),
    _covar(),
    _edep(),
...
void PHG4Showerv2::identify(ostream& os) const {
  os << "---PHG4Showerv2-------------------------------" << endl;
  os << "id: " << get_id() << endl;
  os << "x: " << get_x() << endl;
  os << "y: " << get_y() << endl;
  os << "z: " << get_z() << endl;
...
int PHG4Showerv2::isValid() const {
...
void PHG4Showerv2::set_covar(unsigned int i, unsigned int j, float value) {
  _covar[covar_index(i,j)] = value;
  return;
}

float PHG4Showerv2::get_covar(unsigned int i, unsigned int j) const {
  return _covar[covar_index(i,j)];
}

unsigned int PHG4Showerv2::covar_index(unsigned int i, unsigned int j) const {
  if (i>j) std::swap(i,j);
  return i+1+(j+1)*(j)/2-1;
}

float PHG4Showerv2::get_edep(PHG4Shower::VOLUME volume) const {
  std::map<PHG4Shower::VOLUME,float>::const_iterator citer = _edep.find(volume);
  if (citer == _edep.end()) return 0.0;
  return citer->second;
}

float PHG4Showerv2::get_eion(PHG4Shower::VOLUME volume) const {
  std::map<PHG4Shower::VOLUME,float>::const_iterator citer = _eion.find(volume);
  if (citer == _eion.end()) return 0.0;
  return citer->second;
}

float PHG4Showerv2::get_light_yield(PHG4Shower::VOLUME volume) const {

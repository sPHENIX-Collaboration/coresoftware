#include "RawClusterRecalibrator.h"
...
RawClusterRecalibrator::RawClusterRecalibrator(const std::string &name):
  SubsysReco(string("RawClusterRecalibrator_")+name),
  _calib_params(name)
{
...
int RawClusterRecalibrator::Init(PHCompositeNode *topNode)
{
  CreateNodeTree(topNode);

  if(verbosity){
    std::cout << "RawClusterRecalibrator is running for clusters in the EMCal with parameters:" <<endl;
    _calib_params.Print();
  }
...
int RawClusterRecalibrator::process_event(PHCompositeNode *topNode)
{
  ...
  if(!rawclusters){
    std::cout<<"No CEMC Cluster Container found while in RawClusterRecalibrator, can't proceed!!!"<<std::endl;
    return Fun4AllReturnCodes::ABORTEVENT;
  }

  RawTowerContainer *_towers = findNode::getClass<RawTowerContainer>(topNode,"TOWER_CALIB_CEMC");
  if(!_towers){
    std::cout<<"No calibrated CEMC tower info found while in RawClusterRecalibrator, can't proceed!!!"<<std::endl;
    return Fun4AllReturnCodes::ABORTEVENT;
  }
  ...
  for(iter = begin_end.first; iter!=begin_end.second; ++iter){
    ...
    RawCluster *cluster = 
      dynamic_cast<RawClusterv1*>(iter->second);
    ...
    if(phibin>-1 && etabin>-1)
      recalib_val = 
	_calib_params.get_double_param(calib_const_name.str());
...
void RawClusterRecalibrator::CreateNodeTree(PHCompositeNode *topNode)
{
  ...
  if(!dstNode){
    ...
    throw std::runtime_error("failed to find DST node in RawClusterRecalibrator::CreateNodeTree");
  }
  ...
  if(!_recalib_clusters){
    ...
    PHIODataNode<PHObject> *clusterNode = new PHIODataNode<PHObject>(_recalib_clusters,"CLUSTER_RECALIB_CEMC","PHObject");
...
int RawClusterRecalibrator::End(PHCompositeNode *topNode)
{
  return Fun4AllReturnCodes::EVENT_OK;
}
void RawClusterRecalibrator::SetDefaultParameters(PHG4Parameters &param)
{
